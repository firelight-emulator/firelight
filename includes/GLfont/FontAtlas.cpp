...
FontAtlas::FontAtlas(FT_Face face, int pixelSize)
  ...
  for (int i = 32; i < 128; ++i) {
    ...
    _width += _slot->bitmap.width +
              2; // add the width of this glyph to our texture width
    // Note: We add 2 pixels of blank space between glyphs for padding - this
    // helps reduce texture bleeding
  ...
  for (int i = 32; i < 128; ++i) {
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, texPos, 0, 1, _slot->bitmap.rows, GL_RED,
                    GL_UNSIGNED_BYTE, (char *)0); // padding
    glTexSubImage2D(GL_TEXTURE_2D, 0, texPos, 0, _slot->bitmap.width,
                    _slot->bitmap.rows, GL_RED, GL_UNSIGNED_BYTE,
                    _slot->bitmap.buffer);
    glTexSubImage2D(GL_TEXTURE_2D, 0, texPos, 0, 1, _slot->bitmap.rows, GL_RED,
                    GL_UNSIGNED_BYTE, (char *)0); // padding
...
FontAtlas::~FontAtlas() { glDeleteTextures(1, &_tex); }
