 ...
 *    "1"       - The accelerometer is available as a 3 axis joystick (the
 * default).
 */
...
/**
 *  \brief If set to "0" then never set the top most bit on a SDL Window, even
 * if the video mode expects it. This is a debugging aid for developers and not
...
/**
 * \brief Android APK expansion main file version. Should be a string number
 * like "1", "2" etc.
 *
...
/**
 * \brief Android APK expansion patch file version. Should be a string number
 * like "1", "2" etc.
 *
...
/**
 * \brief A variable to control whether the event loop will block itself when
 * the app is paused.
 *
 * The variable can be set to the following values:
 *   "0"       - Non blocking.
...
/**
 * \brief A variable to control whether we trap the Android back button to
 * handle it manually. This is necessary for the right mouse button to work on
 * some Android devices, or to be able to trap the back button for use in your
 ...
 *   "1"       - Back button will be trapped, allowing you to handle the key
 * press manually.  (This will also let right mouse click work on systems where
 * the right mouse button functions as back.)
 *
 ...
 *
 *  This variable can be set to the following values:
 *    "0"       - Remote orientation does not affect joystick axes (the
 * default). "1"       - Joystick axes are based on the orientation of the
 * remote.
 */
 ...
 * resampling, if available "2" or "medium"  - Use medium quality resampling, if
 * available "3" or "best"    - Use high quality resampling, if available
 */
#define SDL_HINT_AUDIO_RESAMPLING_MODE "SDL_AUDIO_RESAMPLING_MODE"

/**
 *  \brief  A variable controlling whether SDL updates joystick state when
...
/**
 *  \brief  A variable controlling whether SDL updates sensor state when getting
 * input events
 *
 *  This variable can be set to the following values:
 *
...
/**
 *  \brief Prevent SDL from using version 4 of the bitmap header when saving
 * BMPs.
 *
 ...
 * control manually or use asyncify for other purposes) "1"       - Enable
 * emscripten_sleep calls (the default)
 */
#define SDL_HINT_EMSCRIPTEN_ASYNCIFY "SDL_EMSCRIPTEN_ASYNCIFY"

/**
 *  \brief override the binding element for keyboard inputs for Emscripten
 * builds
 *
 * This hint only applies to the emscripten platform
 *
...
/**
 *  \brief  A variable that controls whether the on-screen keyboard should be
...
/**
 *  \brief  A variable that controls whether Steam Controllers should be exposed
...
/**
 *  \brief  A variable controlling verbosity of the logging of SDL events pushed
...
/**
 *  \brief  A variable controlling whether raising the window should be done
 * more forcefully
 *
 *  This variable can be set to the following values:
 ...
 * "security" reasons. See http://stackoverflow.com/a/34414846 for a discussion.
 */
#define SDL_HINT_FORCE_RAISEWINDOW "SDL_HINT_FORCE_RAISEWINDOW"

/**
 *  \brief  A variable controlling how 3D acceleration is used to accelerate the
 * SDL screen surface.
 *
...
/**
 *  \brief  A variable that lets you provide a file with extra gamecontroller db
 * entries.
 *
...
/**
 *  \brief  A variable containing a list of devices to skip when scanning for
 * game controllers.
 *
...
/**
 *  \brief  If set, all devices will be skipped when scanning for game
 * controllers except for the ones listed in this variable.
 *
 *  The format of the string is a comma separated list of USB VID/PID pairs
 *  in hexadecimal form, e.g.
 *
...
/**
 *  \brief  If set, game controller face buttons report their values according
 * to their labels instead of their positional layout.
 *
 ...
 *
 *  The variable can be set to the following values:
 *    "0"       - Report the face buttons by position, as though they were on an
 * Xbox controller. "1"       - Report the face buttons by label instead of
 * position
 *
...
/**
 *  \brief  A variable containing a list of devices to ignore in
 * SDL_hid_enumerate()
 *
...
/**
 * \brief A variable to control whether certain IMEs should handle text editing
...
/**
 * \brief A variable to control whether certain IMEs should show native UI
 * components (such as the Candidate List) instead of suppressing them.
 *
 * The variable can be set to the following values:
 ...
 *
 *  This variable can be set to the following values:
 *    "0"       - The indicator bar is not hidden (default for windowed
 * applications) "1"       - The indicator bar is hidden and is shown when the
 * screen is touched (useful for movie playback applications) "2"       - The
 * indicator bar is dim and the first swipe makes it visible and the second
 * swipe performs the "home" action (default for fullscreen applications)
 */
#define SDL_HINT_IOS_HIDE_HOME_INDICATOR "SDL_IOS_HIDE_HOME_INDICATOR"

/**
 *  \brief  A variable that lets you enable joystick (and gamecontroller) events
...
/**
 *  \brief  A variable controlling whether the HIDAPI joystick drivers should be
 * used.
 *
 *  This variable can be set to the following values:
 ...
 * GameCube controllers should be used.
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether "low_frequency_rumble" and
 ...
 * "high_frequency_rumble" ">0" Rumble is both at any arbitrary value, StopHard
 * is implemented by setting both "low_frequency_rumble" and
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for Nintendo Switch
 ...
 * will be combined into a single Pro-like controller when using the HIDAPI
 * driver
 *
 *  This variable can be set to the following values:
 *    "0"       - Left and right Joy-Con controllers will not be combined and
 * each will be a mini-Gamepad "1"       - Left and right Joy-Con controllers
 * will be combined into a single controller (the default)
 */
...
/**
 *  \brief  A variable controlling whether Nintendo Switch Joy-Con controllers
 * will be in vertical mode when using the HIDAPI driver
 *
 *  This variable can be set to the following values:
 *    "0"       - Left and right Joy-Con controllers will not be in vertical
 * mode (the default) "1"       - Left and right Joy-Con controllers will be in
 * vertical mode
 *
 *  This hint must be set before calling SDL_Init(SDL_INIT_GAMECONTROLLER)
 */
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for Amazon Luna
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for Nintendo Online
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for NVIDIA SHIELD
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for PS3 controllers
 * should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for PS4 controllers
 * should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
...
/**
 *  \brief  A variable controlling whether extended input reports should be used
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for PS5 controllers
 * should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
...
/**
 *  \brief  A variable controlling whether the player LEDs should be lit to
 * indicate which player is associated with a PS5 controller.
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether extended input reports should be used
 * for PS5 controllers when using the HIDAPI driver.
 *
 *  This variable can be set to the following values:
 *    "0"       - extended reports are not enabled (the default)
 *    "1"       - extended reports
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for Google Stadia
 * controllers should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for Bluetooth Steam
 * Controllers should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
...
/**
 *  \brief  A variable controlling whether the Home button LED should be turned
 * on when a Nintendo Switch Pro controller is opened
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether the Home button LED should be turned
 * on when a Nintendo Switch Joy-Con controller is opened
 *
 *  This variable can be set to the following values:
 *    "0"       - home button LED is turned off
 *    "1"       - home button LED is turned on
...
/**
 *  \brief  A variable controlling whether the player LEDs should be lit to
 * indicate which player is associated with a Nintendo Switch controller.
 *
 *  This variable can be set to the following values:
 *    "0"       - player LEDs are not enabled
 *    "1"       - player LEDs are enabled (the default)
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for Nintendo Wii
 * and Wii U controllers should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
 *    "1"       - HIDAPI driver is used
...
/**
 *  \brief  A variable controlling whether the player LEDs should be lit to
 * indicate which player is associated with a Wii controller.
 *
 *  This variable can be set to the following values:
 *    "0"       - player LEDs are not enabled
 *    "1"       - player LEDs are enabled (the default)
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for XBox
 * controllers should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for XBox 360
 * controllers should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
...
/**
 *  \brief  A variable controlling whether the player LEDs should be lit to
 * indicate which player is associated with an Xbox 360 controller.
 *
 *  This variable can be set to the following values:
 *    "0"       - player LEDs are not enabled
 *    "1"       - player LEDs are enabled (the default)
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for XBox 360
 * wireless controllers should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
 *    "1"       - HIDAPI driver is used
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for XBox One
 * controllers should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
...
/**
 *  \brief  A variable controlling whether the Home button LED should be turned
 * on when an Xbox One controller is opened
 *
 *  This variable can be set to the following values:
 *    "0"       - home button LED is turned off
 *    "1"       - home button LED is turned on
...
/**
 *  \brief  A variable controlling whether the RAWINPUT joystick drivers should
 * be used for better handling XInput-capable devices.
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether the RAWINPUT driver should pull
 * correlated data from XInput.
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether the ROG Chakram mice should show up
 * as joysticks
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether Windows.Gaming.Input should be used
 ...
 *    "1"       - SDL Will require DRM master to use the KMSDRM backend
 * (default)
 */
...
/**
 *  \brief  A variable controlling whether joysticks on Linux will always treat
 * 'hat' axis inputs (ABS_HAT0X - ABS_HAT3Y) as 8-way digital hats without
 * checking whether they may be analog.
 *
 *  This variable can be set to the following values:
 *    "0"       - Only map hat axis inputs to digital hat outputs if the input
 * axes appear to actually be digital (the default) "1"       - Always handle
 * the input axes numbered ABS_HAT0X to ABS_HAT3Y as digital hats
 */
#define SDL_HINT_LINUX_DIGITAL_HATS "SDL_LINUX_DIGITAL_HATS"

/**
 *  \brief  A variable controlling whether digital hats on Linux will apply
 * deadzones to their underlying input axes or use unfiltered values.
 *
 *  This variable can be set to the following values:
 *    "0"       - Return digital hat values based on unfiltered input axis
 * values "1"       - Return digital hat values with deadzones on the input axes
 * taken into account (the default)
 */
#define SDL_HINT_LINUX_HAT_DEADZONES "SDL_LINUX_HAT_DEADZONES"

/**
 *  \brief  A variable controlling whether to use the classic /dev/input/js*
 * joystick interface or the newer /dev/input/event* joystick interface on Linux
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether joysticks on Linux adhere to their
...
/**
 *  \brief A variable that determines whether ctrl+click should generate a
...
/**
 *  \brief   A variable controlling whether dispatching OpenGL context updates
 * should block the dispatching thread until the main thread finishes processing
 *
 *  This variable can be set to the following values:
 *    "0"       - Dispatching OpenGL context updates will block the dispatching
 * thread until the main thread finishes processing (default). "1"       -
 * Dispatching OpenGL context updates will allow the dispatching thread to
 * continue execution.
 *
...
/**
 *  \brief  A variable setting the speed scale for mouse motion, in floating
 * point, when the mouse is not in relative mode
 */
#define SDL_HINT_MOUSE_NORMAL_SPEED_SCALE "SDL_MOUSE_NORMAL_SPEED_SCALE"

/**
 *  \brief  A variable controlling whether relative mouse mode constrains the
 ...
 * window
 *
 *  Constraining to the center of the window works better for FPS games and when
 * the application is running over RDP. Constraining to the whole window works
 * better for 2D games and increases the chance that the mouse will be in the
 * correct position when using high DPI mice.
 *
...
/**
 *  \brief  A variable controlling whether relative mouse motion is affected by
 * renderer scaling
 *
 *  This variable can be set to the following values:
 *    "0"       - Relative motion is unaffected by DPI or renderer's logical
 * size "1"       - Relative motion is scaled according to DPI scaling and
 * logical size
 *
...
/**
 *  \brief  A variable setting the scale for mouse motion, in floating point,
 * when the mouse is in relative mode
 */
#define SDL_HINT_MOUSE_RELATIVE_SPEED_SCALE "SDL_MOUSE_RELATIVE_SPEED_SCALE"

/**
 *  \brief  A variable controlling whether the system mouse acceleration curve
 * is used for relative mouse motion.
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether a motion event should be generated
 * for mouse warping in relative mode.
 *
 *  This variable can be set to the following values:
 *    "0"       - Warping the mouse will not generate a motion event in relative
 * mode "1"       - Warping the mouse will generate a motion event in relative
 * mode
 *
...
/**
 *  \brief  A variable controlling whether mouse events should generate
 * synthetic touch events
 *
 *  This variable can be set to the following values:
 *    "0"       - Mouse events will not generate touch events (default for
 * desktop platforms) "1"       - Mouse events will generate touch events
 * (default for mobile platforms, such as Android and iOS)
 */
#define SDL_HINT_MOUSE_TOUCH_EVENTS "SDL_MOUSE_TOUCH_EVENTS"

/**
 *  \brief  A variable controlling whether the mouse is captured while mouse
 * buttons are pressed
 *
 *  This variable can be set to the following values:
 ...
 *
 *  This variable can be set to the following values:
 *    "0"       - Use ES profile of OpenGL, if available. (Default when not
 * set.) "1"       - Load OpenGL ES library using the default library names.
 *
 */
#define SDL_HINT_OPENGL_ES_DRIVER "SDL_OPENGL_ES_DRIVER"

/**
 *  \brief  A variable controlling which orientations are allowed on
 * iOS/Android.
 *
...
/**
 *  \brief  A variable controlling the use of a sentinel event when polling the
 * event queue
 *
 *  This variable can be set to the following values:
 ...
 * SDL_PumpEvents, a poll loop will become stuck until the new events stop. This
 * is most noticeable when moving a high frequency mouse.
 *
...
/**
 *  \brief  A variable describing the content orientation on QtWayland-based
 * platforms.
 *
 ...
 *    "primary" (default), "portrait", "landscape", "inverted-portrait",
 * "inverted-landscape"
 *
...
/**
 *  \brief  Flags to set on QtWayland windows to integrate with the native
 * window manager.
 *
...
/**
 *  \brief  A variable controlling whether the 2D render API is compatible or
 * efficient.
 *
 *  This variable can be set to the following values:
 *
 *    "0"     - Don't use batching to make rendering more efficient.
 *    "1"     - Use batching, but might cause problems if app makes its own
 * direct OpenGL calls.
 *
 ...
 * as of SDL 2.0.20) "1"     - Use the driver point API using Bresenham's line
 * algorithm (correct, draws many points) "2"     - Use the driver line API
 * (occasionally misses line endpoints based on hardware driver quirks, was the
 * default before 2.0.20) "3"     - Use the driver geometry API (correct, draws
...
/**
 *  \brief  A variable controlling whether the Direct3D device is initialized
...
/**
 *  \brief  A variable controlling the scaling policy for
 * SDL_RenderSetLogicalSize.
 *
 *  This variable can be set to the following values:
 *    "0" or "letterbox" - Uses letterbox/sidebars to fit the entire rendering
 * on screen "1" or "overscan"  - Will zoom the rendering so it fills the entire
...
/**
 *  \brief  A variable controlling whether the OpenGL render driver uses shaders
...
/**
 *  \brief  A variable controlling whether the Metal render driver select low
 * power device over default one
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling if VSYNC is automatically disable if doesn't
 * reach the enough FPS
 *
 *  This variable can be set to the following values:
 ...
 *
 * The variable can be set to the following values:
 *   "0"       - The return key will be handled as a key event. This is the
 * behaviour of SDL <= 2.0.3. (default) "1"       - The return key will hide the
 * keyboard.
 *
 * The value of this hint is used at runtime, so it can be changed at any time.
 */
...
/**
 *  \brief Specifies whether SDL_THREAD_PRIORITY_TIME_CRITICAL should be treated
 * as realtime.
 *
 ...
 *    "1"       - Force SDL_THREAD_PRIORITY_TIME_CRITICAL to a realtime
 * scheduling policy
 */
...
/**
 *  \brief  A string specifying additional information to use with
 ...
 * future.
 *
 *  \note On Linux, the kernel may send SIGKILL to realtime tasks which exceed
 * the distro configured execution budget for rtkit. This budget can be queried
 * through RLIMIT_RTTIME after calling SDL_SetThreadPriority().
 */
#define SDL_HINT_THREAD_PRIORITY_POLICY "SDL_THREAD_PRIORITY_POLICY"

/**
 *  \brief  A string specifying SDL's threads stack size in bytes or "0" for the
 ...
 * stack size (a few kilobytes versus the default 8MB glibc uses). Support for
 * this hint is currently available only in the pthread, Windows, and PSP
 * backend.
 *
...
/**
 *  \brief  A variable controlling whether touch events should generate
...
/**
 *  \brief  A variable controlling which touchpad should generate synthetic
 * mouse events
 *
 *  This variable can be set to the following values:
 ...
 *    "1"       - Remotes are available as 2 axis, 2 button joysticks (the
 * default).
 */
...
/**
 * \brief A variable controlling whether the graphics context is externally
 * managed.
 *
 * This variable can be set to the following values:
 *  "0"         - SDL will manage graphics contexts that are attached to
 * windows. "1"         - Disable graphics context management on windows.
 *
 * By default SDL will manage OpenGL contexts in certain situations. For
 * example, on Android the context will be automatically saved and restored when
 * pausing the application. Additionally, some platforms will assume usage of
 * OpenGL if Vulkan isn't used. Setting this to "1" will prevent this behavior,
 * which is desireable when the application manages the graphics context, such
 * as an externally managed OpenGL context or attaching a Vulkan surface to the
 * window.
 */
#define SDL_HINT_VIDEO_EXTERNAL_CONTEXT "SDL_VIDEO_EXTERNAL_CONTEXT"

/**
 *  \brief If set to 1, then do not allow high-DPI windows. ("Retina" on Mac and
 * iOS)
 */
...
/**
 *  \brief Minimize your SDL_Window if it loses key focus when in fullscreen
 * mode. Defaults to false. \warning  Before SDL 2.0.14, this defaulted to true!
 * In 2.0.14, we're seeing if "true" causes more problems than it solves in
 * modern times.
 *
 */
...
/**
 *  \brief  A variable controlling whether the libdecor Wayland backend is
 * allowed to be used.
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether the libdecor Wayland backend is
 * preferred over native decrations.
 *
 ...
 *
 *  This variable can be set to the following values:
 *    "0"       - libdecor is enabled only if server-side decorations are
 * unavailable. "1"       - libdecor is always enabled if available.
 *
 *  libdecor is used over xdg-shell when xdg-decoration protocol is unavailable.
 */
...
/**
 *  \brief  A variable controlling whether video mode emulation is enabled under
 * Wayland.
 *
 *  When this hint is set, a standard set of emulated CVT video modes will be
 * exposed for use by the application. If it is disabled, the only modes exposed
 * will be the logical desktop size and, in the case of a scaled desktop, the
...
/**
 *  \brief  Enable or disable mouse pointer warp emulation, needed by some older
 * games.
 *
 *  When this hint is set, any SDL will emulate mouse warps using relative mouse
 * mode. This is required for some older games (such as Source engine games),
 * which warp the mouse to the centre of the screen rather than using relative
 * mouse motion. Note that relative mouse mode may have different mouse
 * acceleration behaviour than pointer warps.
 *
 *  This variable can be set to the following values:
 *    "0"       - All mouse warps fail, as mouse warping is not available under
 * wayland. "1"       - Some mouse warps will be emulated by forcing relative
 * mouse mode.
 *
...
/**
 *  \brief  A variable that is the address of another SDL_Window* (as a hex
 ...
 *
 *  This variable can be set to the following values:
 *    The address (as a string "%p") of the SDL_Window* that new windows created
 * with SDL_CreateWindowFrom() should share a pixel format with.
 */
#define SDL_HINT_VIDEO_WINDOW_SHARE_PIXEL_FORMAT                               \
  "SDL_VIDEO_WINDOW_SHARE_PIXEL_FORMAT"

/**
 *  \brief  When calling SDL_CreateWindowFrom(), make the window compatible with
 * OpenGL.
 *
 * This variable can be set to the following values:
...
/**
 *  \brief  A variable specifying which shader compiler to preload when using
...
/**
 * \brief A variable controlling whether the X11 _NET_WM_BYPASS_COMPOSITOR hint
 * should be used.
 *
 * This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether the X11 _NET_WM_PING protocol should
 * be supported.
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A no-longer-used variable controlling whether the X11 Xinerama
 * extension should be used.
 *
 * Before SDL 2.0.24, this would let apps and users disable Xinerama support on
 * X11. Now SDL never uses Xinerama, and does not check for this hint at all.
 *  The preprocessor define is left here for source compatibility.
 */
#define SDL_HINT_VIDEO_X11_XINERAMA "SDL_VIDEO_X11_XINERAMA"

/**
 *  \brief  A variable controlling whether the X11 XRandR extension should be
 * used.
 *
 *  This variable can be set to the following values:
 *    "0"       - Disable XRandR
...
/**
 *  \brief  A no-longer-used variable controlling whether the X11 VidMode
 * extension should be used.
 *
 * Before SDL 2.0.24, this would let apps and users disable XVidMode support on
 * X11. Now SDL never uses XVidMode, and does not check for this hint at all.
 *  The preprocessor define is left here for source compatibility.
 */
...
/**
 *  \brief  Controls how the size of the RIFF chunk affects the loading of a
 * WAVE file.
 *
 ...
 *  This variable can be set to the following values:
 *
 *    "force"        - Always use the RIFF chunk size as a boundary for the
 * chunk search "ignorezero"   - Like "force", but a zero size searches up to 4
 * GiB (default) "ignore"       - Ignore the RIFF chunk size and always search
 * up to 4 GiB "maximum"      - Search for chunks until the end of file (not
 * recommended)
 */
 ...
 * ignored "dropframe"  - Decode until the first incomplete sample frame
...
/**
 *  \brief Controls whether menus can be opened with their keyboard shortcut
 * (Alt+mnemonic).
 *
...
/**
 *  \brief  A variable controlling whether the windows message loop is processed
 * by SDL
 *
 *  This variable can be set to the following values:
 ...
 * Sections. (default) "1"       - Force the use of Critical Sections in all
 * cases.
 *
 */
 ...
 *
 *  This variable can be set to the following values:
 *    "0"       - Use Atomics and WaitOnAddress API when available. If not, fall
 * back to Kernel Objects. (default) "1"       - Force the use of Kernel Objects
 * in all cases.
 *
 */
...
/**
 * \brief A variable to specify custom icon resource id from RC file on Windows
 * platform
 */
...
/**
 * \brief Use the D3D9Ex API introduced in Windows Vista, instead of normal
 * D3D9. Direct3D 9Ex contains changes to state management that can eliminate
 * device loss errors during scenarios like Alt+Tab or UAC prompts. D3D9Ex may
 ...
 *    "1"       - Use the Direct3D 9Ex API on Vista and later (and fall back if
 * D3D9Ex is unavailable)
 *
 */
 ...
 *
 *  This hint is equivalent to requesting DPI awareness via external means (e.g.
 * calling SetProcessDpiAwarenessContext) and does not cause SDL to use a
 * virtualized coordinate system, so it will generally give you 1 SDL coordinate
 * = 1 pixel even on high-DPI displays.
 *
 ...
 *    "unaware"      - Declare the process as DPI unaware. (Windows 8.1 and
 * later). "system"       - Request system DPI awareness. (Vista and later).
 *    "permonitor"   - Request per-monitor DPI awareness. (Windows 8.1 and
 * later). "permonitorv2" - Request per-monitor V2 DPI awareness. (Windows 10,
 ...
 * dragging between monitors with different scale factors. This is the preferred
 * DPI awareness level.
 *
 ...
 *
 *  This changes the SDL coordinate system units to be DPI-scaled points, rather
 * than pixels everywhere. This means windows will be appropriately sized, even
 * when created on high-DPI displays with scaling.
 *
 *  e.g. requesting a 640x480 window from SDL, on a display with 125% scaling in
 * Windows display settings, will create a window with an 800x600 client area
 * (in pixels).
 *
 ...
 * resizing when dragging between monitors with different scale factors (unless
 * this is performed by Windows itself, which is the case when the process is
 * DPI unaware). "1"       - SDL coordinates are in DPI-scaled points.
 * Automatically resize windows as needed on displays with non-100% scale
 * factors.
 */
#define SDL_HINT_WINDOWS_DPI_SCALING "SDL_WINDOWS_DPI_SCALING"

/**
 *  \brief  A variable controlling whether the window frame and title bar are
 * interactive when the cursor is hidden
 *
 *  This variable can be set to the following values:
 *    "0"       - The window frame is not interactive when the cursor is hidden
 * (no move, resize, etc) "1"       - The window frame is interactive when the
 * cursor is hidden
 *
...
/**
 *  \brief  A variable controlling whether the window is activated when the
 * SDL_ShowWindow function is called
 *
 *  This variable can be set to the following values:
 *    "0"       - The window is activated when the SDL_ShowWindow function is
 * called "1"       - The window is not activated when the SDL_ShowWindow
 * function is called
 *
...
/** \brief Allows back-button-press events on Windows Phone to be marked as
 * handled
 *
 ...
 * SDL_HINT_WINRT_HANDLE_BACK_BUTTON set by such a callback, will be applied to
 * the OS' current back-button-press event.
 *
 ...
 * 8, 8.1, and RT, Microsoft mandates that this policy be available via the
 * Windows UserSettings charm. SDL provides code to add a link there, with its
 ...
 *  be available in the Windows UserSettings charm, as accessed from within the
 * app. SDL provides code to add a URL-based link there, which can point to the
 ...
 *  link to the UserSettings charm.  This hint should only be set during app
 * init.
 *
...
/**
 *  \brief  A variable that lets you disable the detection and use of Xinput
 * Gamepad devices
 *
 *  The variable can be set to the following values:
...
/**
 *  \brief  A variable that causes SDL to use the old axis and button mapping
 * for XInput devices.
 *
...
/**
 *  \brief  A variable that decides whether to send SDL_QUIT when closing the
 * final window.
 *
