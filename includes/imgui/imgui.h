...
ImGuiSizeCallbackData, ImGuiPayload, ImGuiTableSortSpecs,
...
struct ImGuiPlatformImeData; // Platform IME data for io.SetPlatformImeDataFn()
                             // function.
...
typedef int
    ImGuiDir; // -> enum ImGuiDir_             // Enum: A cardinal direction
...
typedef int ImGuiMouseCursor; // -> enum ImGuiMouseCursor_     // Enum: A mouse
                              // cursor shape
typedef int ImGuiSortDirection; // -> enum ImGuiSortDirection_   // Enum: A
...
// Flags (declared as int for compatibility with old C++, to allow using as
// flags without overhead, and to not pollute the top of this file)
// - Tip: Use your programming IDE navigation facilities on the names in the
// _central column_ below to find the actual flags/enum lists!
//   In Visual Studio IDE: CTRL+comma ("Edit.GoToAll") can follow symbols in
//   comments, whereas CTRL+F12 ("Edit.GoToImplementation") cannot. With Visual
...
typedef int ImGuiKeyChord;       // -> ImGuiKey | ImGuiMod_XXX    // Flags: for
                           // storage only for now: an ImGuiKey optionally OR-ed
                           // with one or more ImGuiMod_XXX values.
typedef int
...
//   regardless of its return value! [Important: due to legacy reason, this is
//   inconsistent with most other functions such as BeginMenu/EndMenu,
//    BeginPopup/EndPopup, etc. where the EndXXX call should only be called if
//    the corresponding BeginXXX function returned true. Begin and BeginChild
...
// their own child.
// - For each independent axis of 'size': ==0.0f: use remaining host window size
// / >0.0f: fixed size / <0.0f: use remaining window size minus abs(size) / Each
// axis can use a different mode, e.g. ImVec2(0,400).
// - BeginChild() returns false to indicate the window is collapsed or fully
// clipped, so you may early out and omit submitting anything to the window.
//   Always call a matching EndChild() for each BeginChild() call, regardless of
//   its return value. [Important: due to legacy reason, this is inconsistent
//   with most other functions such as BeginMenu/EndMenu,
//    BeginPopup/EndPopup, etc. where the EndXXX call should only be called if
//    the corresponding BeginXXX function returned true. Begin and BeginChild
//    are the only odd ones out. Will be fixed in a future update.]
...
IMGUI_API void PushStyleVar(
    ImGuiStyleVar idx,
    const ImVec2 &val); // modify a style ImVec2 variable. always use this if
                        // you modify the style after NewFrame().
...
IMGUI_API ImU32
GetColorU32(ImU32 col); // retrieve given color with style alpha applied, packed
                        // as a 32-bit value suitable for ImDrawList
...
IMGUI_API void SetCursorScreenPos(
    const ImVec2 &pos);          // cursor position in absolute coordinates
IMGUI_API ImVec2 GetCursorPos(); // [window-local] cursor position in window
...
IMGUI_API ImVec2 GetCursorStartPos(); // [window-local] initial cursor position,
...
GetID(const char *str_id); // calculate unique ID (hash of whole ID stack +
                           // given parameter). e.g. if you want to query into
                           // ImGuiStorage yourself
...
IMGUI_API void LabelText(const char *label, const char *fmt, ...) IM_FMTARGS(
...
IMGUI_API bool SmallButton(const char *label); // button with (FramePadding.y ==
...
IMGUI_API bool ArrowButton(const char *str_id,
...
IMGUI_API bool RadioButton(const char *label, int *v,
                           int v_button); // shortcut to handle the above
...
IMGUI_API bool ImageButton(const char *str_id, ImTextureID user_texture_id,
...
// ImGuiSliderFlags_AlwaysClamp to always clamp.
...
// manual input can override those limits if ImGuiSliderFlags_AlwaysClamp is not
// used.
...
// `float power=1.0f' argument instead of the `ImGuiSliderFlags flags=0'
// argument.
...
IMGUI_API bool DragFloat2(const char *label, float v[2], float v_speed = 1.0f,
                          float v_min = 0.0f, float v_max = 0.0f,
                          const char *format = "%.3f",
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool DragFloat3(const char *label, float v[3], float v_speed = 1.0f,
                          float v_min = 0.0f, float v_max = 0.0f,
                          const char *format = "%.3f",
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool DragFloat4(const char *label, float v[4], float v_speed = 1.0f,
                          float v_min = 0.0f, float v_max = 0.0f,
                          const char *format = "%.3f",
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool DragFloatRange2(const char *label, float *v_current_min,
...
DragInt(const char *label, int *v, float v_speed = 1.0f, int v_min = 0,
        ...
        ImGuiSliderFlags flags = 0); // If v_min >= v_max we have no bound
IMGUI_API bool DragInt2(const char *label, int v[2], float v_speed = 1.0f,
                        int v_min = 0, int v_max = 0, const char *format = "%d",
                        ImGuiSliderFlags flags = 0);
IMGUI_API bool DragInt3(const char *label, int v[3], float v_speed = 1.0f,
                        int v_min = 0, int v_max = 0, const char *format = "%d",
                        ImGuiSliderFlags flags = 0);
IMGUI_API bool DragInt4(const char *label, int v[4], float v_speed = 1.0f,
                        int v_min = 0, int v_max = 0, const char *format = "%d",
                        ImGuiSliderFlags flags = 0);
IMGUI_API bool DragIntRange2(const char *label, int *v_current_min,
                             ...
                             const char *format_max = NULL,
                             ImGuiSliderFlags flags = 0);
IMGUI_API bool DragScalar(const char *label, ImGuiDataType data_type,
                          void *p_data, float v_speed = 1.0f,
                          const void *p_min = NULL, const void *p_max = NULL,
                          const char *format = NULL,
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool DragScalarN(const char *label, ImGuiDataType data_type,
...
// final `float power=1.0f' argument instead of the `ImGuiSliderFlags flags=0'
// argument.
//   If you get a warning converting a float to ImGuiSliderFlags, read
//   https://github.com/ocornut/imgui/issues/3361
IMGUI_API bool
SliderFloat(const char *label, float *v, float v_min, float v_max,
            ...
            ImGuiSliderFlags flags =
                0); // adjust format to decorate the value with a prefix or a
                    // suffix for in-slider labels or unit display.
IMGUI_API bool SliderFloat2(const char *label, float v[2], float v_min,
                            float v_max, const char *format = "%.3f",
                            ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderFloat3(const char *label, float v[3], float v_min,
                            float v_max, const char *format = "%.3f",
                            ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderFloat4(const char *label, float v[4], float v_min,
                            float v_max, const char *format = "%.3f",
                            ImGuiSliderFlags flags = 0);
...
IMGUI_API bool SliderInt2(const char *label, int v[2], int v_min, int v_max,
                          const char *format = "%d",
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderInt3(const char *label, int v[3], int v_min, int v_max,
                          const char *format = "%d",
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderInt4(const char *label, int v[4], int v_min, int v_max,
                          const char *format = "%d",
                          ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderScalar(const char *label, ImGuiDataType data_type,
                            void *p_data, const void *p_min, const void *p_max,
                            const char *format = NULL,
                            ImGuiSliderFlags flags = 0);
IMGUI_API bool SliderScalarN(const char *label, ImGuiDataType data_type,
...
IMGUI_API bool VSliderScalar(const char *label, const ImVec2 &size,
                             ImGuiDataType data_type, void *p_data,
...
IMGUI_API bool InputTextMultiline(const char *label, char *buf, size_t buf_size,
                                  ...
                                  ImGuiInputTextCallback callback = NULL,
                                  void *user_data = NULL);
IMGUI_API bool InputTextWithHint(const char *label, const char *hint, char *buf,
                                 size_t buf_size, ImGuiInputTextFlags flags = 0,
                                 ImGuiInputTextCallback callback = NULL,
                                 void *user_data = NULL);
IMGUI_API bool InputFloat(const char *label, float *v, float step = 0.0f,
                          float step_fast = 0.0f, const char *format = "%.3f",
                          ImGuiInputTextFlags flags = 0);
IMGUI_API bool InputFloat2(const char *label, float v[2],
                           const char *format = "%.3f",
                           ImGuiInputTextFlags flags = 0);
IMGUI_API bool InputFloat3(const char *label, float v[3],
                           const char *format = "%.3f",
                           ImGuiInputTextFlags flags = 0);
IMGUI_API bool InputFloat4(const char *label, float v[4],
                           const char *format = "%.3f",
                           ImGuiInputTextFlags flags = 0);
IMGUI_API bool InputInt(const char *label, int *v, int step = 1,
...
IMGUI_API bool InputInt4(const char *label, int v[4],
                         ImGuiInputTextFlags flags = 0);
IMGUI_API bool InputDouble(const char *label, double *v, double step = 0.0,
                           double step_fast = 0.0, const char *format = "%.6f",
                           ImGuiInputTextFlags flags = 0);
IMGUI_API bool InputScalar(const char *label, ImGuiDataType data_type,
                           void *p_data, const void *p_step = NULL,
                           const void *p_step_fast = NULL,
                           const char *format = NULL,
                           ImGuiInputTextFlags flags = 0);
IMGUI_API bool InputScalarN(const char *label, ImGuiDataType data_type,
...
IMGUI_API bool ColorButton(const char *desc_id, const ImVec4 &col,
                           ...
                           const ImVec2 &size = ImVec2(
                               0, 0)); // display a color square/button, hover
...
IMGUI_API bool TreeNodeEx(const char *str_id, ImGuiTreeNodeFlags flags,
                          const char *fmt, ...) IM_FMTARGS(3);
IMGUI_API bool TreeNodeEx(const void *ptr_id, ImGuiTreeNodeFlags flags,
                          const char *fmt, ...) IM_FMTARGS(3);
IMGUI_API bool TreeNodeExV(const char *str_id, ImGuiTreeNodeFlags flags,
                           const char *fmt, va_list args) IM_FMTLIST(3);
IMGUI_API bool TreeNodeExV(const void *ptr_id, ImGuiTreeNodeFlags flags,
                           const char *fmt, va_list args) IM_FMTLIST(3);
IMGUI_API void
...
Selectable(const char *label, bool *p_selected, ImGuiSelectableFlags flags = 0,
           const ImVec2 &size =
               ImVec2(0, 0)); // "bool* p_selected" point to the selection state
                              // (read-write), as a convenient helper.

// Widgets: List Boxes
// - This is essentially a thin wrapper to using BeginChild/EndChild with some
// stylistic changes.
// - The BeginListBox()/EndListBox() api allows you to manage your contents and
// selection state however you want it, by creating e.g. Selectable() or any
// items.
...
// better!
IMGUI_API void PlotLines(const char *label, const float *values,
...
IMGUI_API void
PlotHistogram(const char *label, float (*values_getter)(void *data, int idx),
              void *data, int values_count, int values_offset = 0,
...
// to handle your types. you can add functions to the ImGui namespace)
...
IMGUI_API bool
BeginMenuBar(); // append to menu-bar of current window (requires
...
IMGUI_API void EndMainMenuBar(); // only call EndMainMenuBar() if
...
IMGUI_API void EndTooltip();   // only call EndTooltip() if
                             // BeginTooltip()/BeginItemTooltip() returns true!
IMGUI_API void SetTooltip(const char *fmt, ...) IM_FMTARGS(
    1); // set a text-only tooltip. Often used after a ImGui::IsItemHovered()
        // check. Override any previous call to SetTooltip().
IMGUI_API void SetTooltipV(const char *fmt, va_list args) IM_FMTLIST(1);

// Tooltips: helpers for showing a tooltip when hovering an item
// - BeginItemTooltip() is a shortcut for the 'if
// (IsItemHovered(ImGuiHoveredFlags_ForTooltip) && BeginTooltip())' idiom.
// - SetItemTooltip() is a shortcut for the 'if
// (IsItemHovered(ImGuiHoveredFlags_ForTooltip)) { SetTooltip(...); }' idiom.
// - Where 'ImGuiHoveredFlags_ForTooltip' itself is a shortcut to use
...
IMGUI_API bool BeginItemTooltip(); // begin/append a tooltip window if preceding
                                   // item was hovered.
IMGUI_API void SetItemTooltip(const char *fmt, ...)
    IM_FMTARGS(1); // set a text-only tooltip if preceeding item was hovered.
...
BeginPopup(const char *str_id,
           ImGuiWindowFlags flags = 0); // return true if the popup is open, and
...
//  - OpenPopup(): set popup state to open. ImGuiPopupFlags are available for
//  opening options.
//  - If not modal: they can be closed by clicking anywhere outside them, or by
//  pressing ESCAPE.
...
//  - Use ImGuiPopupFlags_NoOpenOverExistingPopup to avoid opening a popup if
...
//  flags to 1 (== ImGuiPopupFlags_MouseButtonRight) for backward compatibility
...
//  ImGuiWindowFlags to the BeginPopupContextXXX functions in the future.
//  - IMPORTANT: Notice that we exceptionally default their flags to 1 (==
//  ImGuiPopupFlags_MouseButtonRight) for backward compatibility with older API
//  taking 'int mouse_button = 1' parameter, so if you add other flags remember
//  to re-add the ImGuiPopupFlags_MouseButtonRight.
...
//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId: return true if any popup is
//  open at the current BeginPopup() level of the popup stack.
//  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId +
//  ImGuiPopupFlags_AnyPopupLevel: return true if any popup is open.
IMGUI_API bool
...
// commentary.
// - See ImGuiTableFlags_ and ImGuiTableColumnFlags_ enums for a description of
...
//        TableSetColumnIndex(1) -> Text("Hello 1")  // OK TableNextRow() ->
//        TableNextColumn()      -> Text("Hello 0") -> TableNextColumn()      ->
//        Text("Hello 1")  // OK
//                          TableNextColumn()      -> Text("Hello 0") ->
//                          TableNextColumn()      -> Text("Hello 1")  // OK:
//                          TableNextColumn() automatically gets to next row!
//        TableNextRow()                           -> Text("Hello 0") // Not OK!
//        Missing TableSetColumnIndex() or TableNextColumn()! Text will not
//        appear!
//        --------------------------------------------------------------------------------------------------------
// - 5. Call EndTable()
IMGUI_API bool BeginTable(const char *str_id, int column,
...
IMGUI_API bool
TableNextColumn(); // append into the next column (or first column of next row
...
IMGUI_API void TableSetupColumn(const char *label,
                                ...
                                ImGuiID user_id = 0);
IMGUI_API void TableSetupScrollFreeze(
    int cols,
...
TableAngledHeadersRow(); // submit a row with angled headers for every column
                         // with the ImGuiTableColumnFlags_AngledHeader flag.
                         // MUST BE FIRST ROW.
...
IMGUI_API void Columns(int count = 1, const char *id = NULL,
                       bool border = true);
...
IMGUI_API bool
TabItemButton(const char *label,
              ImGuiTabItemFlags flags =
                  0); // create a Tab behaving like a button. return true when
...
IMGUI_API const ImGuiPayload *AcceptDragDropPayload(
    ...
    ImGuiDragDropFlags flags =
        0); // accept contents of a given type. If
            // ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek into
            // the payload before the mouse button is released.
IMGUI_API void EndDragDropTarget(); // only call EndDragDropTarget() if
...
GetDragDropPayload(); // peek directly into the current payload from anywhere.
                      ...
                      // inactive. use ImGuiPayload::IsDataType() to test for
                      // the payload type.
...
// calls to ImDrawList::PushClipRect() which are render only.
...
IMGUI_API bool IsItemVisible(); // is the last item visible? (items may be out
...
IMGUI_API bool IsItemActivated(); // was the last item just made active (item
...
IMGUI_API ImVec2 GetItemRectMin();  // get upper-left bounding rectangle of the
                                    // last item (screen space)
IMGUI_API ImVec2 GetItemRectMax();  // get lower-right bounding rectangle of the
                                    // last item (screen space)
IMGUI_API ImVec2 GetItemRectSize(); // get size of last item

// Viewports
// - Currently represents the Platform Window created by the application which
// is hosting our Dear ImGui windows.
...
GetBackgroundDrawList(); // this draw list will be the first rendered one.
                         // Useful to quickly draw shapes/text behind dear imgui
                         // contents.
IMGUI_API ImDrawList *
GetForegroundDrawList(); // this draw list will be the last rendered one. Useful
                         // to quickly draw shapes/text over dear imgui
                         // contents.

// Miscellaneous Utilities
IMGUI_API bool IsRectVisible(
    const ImVec2 &size); // test if rectangle (of given size, starting from
...
GetDrawListSharedData(); // you may use this when creating your own ImDrawList
                         // instances.
IMGUI_API const char *
GetStyleColorName(ImGuiCol idx); // get a string corresponding to the enum value
...
IMGUI_API void
EndChildFrame(); // always call EndChildFrame() regardless of BeginChildFrame()
                 // return values (which indicates a collapsed/clipped window)

// Text Utilities
IMGUI_API ImVec2 CalcTextSize(const char *text, const char *text_end = NULL,
...
IMGUI_API bool
IsKeyReleased(ImGuiKey key); // was key released (went from Down to !Down)?
IMGUI_API bool IsKeyChordPressed(
    ImGuiKeyChord
        key_chord); // was key chord (mods + key) pressed, e.g. you can pass
                    // 'ImGuiMod_Ctrl | ImGuiKey_S' as a key-chord. This doesn't
                    // do any routing or focus check, please consider using
...
IMGUI_API bool IsMouseClicked(
    ImGuiMouseButton button,
    bool repeat = false); // did mouse button clicked? (went from !Down to
                          // Down). Same as GetMouseClickedCount() == 1.
IMGUI_API bool
IsMouseReleased(ImGuiMouseButton button); // did mouse button released? (went
                                          // from Down to !Down)
...
IMGUI_API int GetMouseClickedCount(
    ImGuiMouseButton button); // return the number of successive mouse-clicks at
...
IMGUI_API bool
IsMousePosValid(const ImVec2 *mouse_pos =
                    NULL); // by convention we use (-FLT_MAX,-FLT_MAX) to denote
...
IMGUI_API ImVec2 GetMousePos(); // shortcut to ImGui::GetIO().MousePos provided
                                // by user, to be consistent with other calls
IMGUI_API ImVec2
...
GetMouseCursor(); // get desired mouse cursor shape. Important: reset in
                  // ImGui::NewFrame(), this is updated during the frame. valid
                  // before Render(). If you use software rendering by setting
                  // io.MouseDrawCursor ImGui will render those for you
IMGUI_API void
SetMouseCursor(ImGuiMouseCursor cursor_type); // set desired mouse cursor shape
IMGUI_API void SetNextFrameWantCaptureMouse(
    bool
...
// (Those are per-window flags. There are shared flags in ImGuiIO:
...
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_NoScrollbar = 1
                                 << 3, // Disable scrollbars (window can still
  ...
  ImGuiWindowFlags_NoCollapse =
      1 << 5, // Disable user collapsing window by double-clicking on it. Also
              // referred to as Window QuickMenu Button (e.g. within a docking
              // node).
  ...
  ImGuiWindowFlags_HorizontalScrollbar =
      1 << 11, // Allow horizontal scrollbar to appear (off by default). You may
               // use SetNextWindowContentSize(ImVec2(width,0.0f)); prior to
               // calling Begin() to specify width. Read code in imgui_demo in
  ...
  ImGuiWindowFlags_UnsavedDocument =
      1 << 20, // Display a dot next to the title. When used in a tab/docking
  ...
  ImGuiWindowFlags_NavFlattened =
      1 << 23, // [BETA] On child window: allow gamepad/keyboard navigation to
               // cross over parent border to this child or between sibling
               // child windows.
  ImGuiWindowFlags_ChildWindow =
      1 << 24, // Don't use! For internal use by BeginChild()
  ImGuiWindowFlags_Tooltip =
      1 << 25, // Don't use! For internal use by BeginTooltip()
  ImGuiWindowFlags_Popup = 1
                           << 26, // Don't use! For internal use by BeginPopup()
...
// (Those are per-item flags. There are shared flags in ImGuiIO:
...
enum ImGuiTreeNodeFlags_ {
  ...
  ImGuiTreeNodeFlags_OpenOnDoubleClick = 1
                                         << 6, // Need double-click to open node
  ...
  // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 14,  // FIXME: TODO: Disable
  ...
  ImGuiTreeNodeFlags_AllowItemOverlap =
      ImGuiTreeNodeFlags_AllowOverlap, // Renamed in 1.89.7
#endif
};
...
//   to pass a mouse button index in ImGuiPopupFlags.
// - For the same reason, we exceptionally default the ImGuiPopupFlags argument
// of BeginPopupContextXXX functions to 1 instead of 0.
//   IMPORTANT: because the default parameter is 1
//   (==ImGuiPopupFlags_MouseButtonRight), if you rely on the default parameter
...
enum ImGuiPopupFlags_ {
  ...
  ImGuiPopupFlags_AnyPopupId = 1 << 7, // For IsPopupOpen(): ignore the ImGuiID
...
enum ImGuiSelectableFlags_ {
  ...
  ImGuiSelectableFlags_AllowItemOverlap =
      ImGuiSelectableFlags_AllowOverlap, // Renamed in 1.89.7
#endif
};
...
enum ImGuiTabBarFlags_ {
  ...
  ImGuiTabBarFlags_NoCloseWithMiddleMouseButton =
      1 << 3, // Disable behavior of closing tabs (that are submitted with
              // p_open != NULL) with middle mouse button. You can still repro
              // this behavior on user's side with if (IsItemHovered() &&
...
enum ImGuiTabItemFlags_ {
  ...
  ImGuiTabItemFlags_UnsavedDocument =
      1 << 0, // Display a dot next to the title + tab is selected when clicking
              // the X + closure is not assumed (will wait for user to stop
              // submitting the tab). Otherwise closure is assumed when pressing
              // the X, so if you keep submitting the tab may reappear at end of
              // tab bar.
  ImGuiTabItemFlags_SetSelected =
      1 << 1, // Trigger flag to programmatically make the tab selected when
  ...
  ImGuiTabItemFlags_NoPushId =
      1
      << 3, // Don't call PushID(tab->ID)/PopID() on BeginTabItem()/EndTabItem()
  ImGuiTabItemFlags_NoTooltip = 1 << 4, // Disable tooltip for the given tab
  ImGuiTabItemFlags_NoReorder = 1 << 5, // Disable reordering this tab or having
                                        // another tab cross over this tab
  ImGuiTabItemFlags_Leading =
      1 << 6, // Enforce the tab position to the left of the tab bar (after the
              // tab list popup button)
  ImGuiTabItemFlags_Trailing =
      1 << 7, // Enforce the tab position to the right of the tab bar (before
...
// - When ScrollX is off:
//    - Table defaults to ImGuiTableFlags_SizingStretchSame -> all Columns
//    defaults to ImGuiTableColumnFlags_WidthStretch with same weight.
...
// - When ScrollX is on:
//    - Table defaults to ImGuiTableFlags_SizingFixedFit -> all Columns defaults
//    to ImGuiTableColumnFlags_WidthFixed
...
enum ImGuiTableFlags_ {
  ...
  ImGuiTableFlags_Sortable =
      1 << 3, // Enable sorting. Call TableGetSortSpecs() to obtain sort specs.
              // Also see ImGuiTableFlags_SortMulti and
  ...
  ImGuiTableFlags_BordersInnerH = 1
                                  << 7, // Draw horizontal borders between rows.
  ...
  ImGuiTableFlags_BordersH =
      ...
      ImGuiTableFlags_BordersOuterH, // Draw horizontal borders.
  ImGuiTableFlags_BordersV =
      ImGuiTableFlags_BordersInnerV |
      ImGuiTableFlags_BordersOuterV, // Draw vertical borders.
  ImGuiTableFlags_BordersInner =
      ImGuiTableFlags_BordersInnerV |
      ImGuiTableFlags_BordersInnerH, // Draw inner borders.
  ImGuiTableFlags_BordersOuter =
      ImGuiTableFlags_BordersOuterV |
      ImGuiTableFlags_BordersOuterH, // Draw outer borders.
  ImGuiTableFlags_Borders = ImGuiTableFlags_BordersInner |
                            ImGuiTableFlags_BordersOuter, // Draw all borders.
...
// When using ImGuiTableFlags_RowBg on the table, each row has the RowBg0 color
...
enum ImGuiFocusedFlags_ {
  ...
  // ImGuiFocusedFlags_DockHierarchy               = 1 << 4,   // Consider
...
// Dear ImGui or to your app, you should use 'io.WantCaptureMouse' instead!
// Please read the FAQ! Note: windows with the ImGuiWindowFlags_NoInputs flag
// are ignored by IsWindowHovered() calls.
enum ImGuiHoveredFlags_ {
  ImGuiHoveredFlags_None =
      0, // Return true if directly over the item/window, not obstructed by
         // another window, not obstructed by an active popup or modal blocking
         // inputs under them.
  ImGuiHoveredFlags_ChildWindows =
      1 << 0, // IsWindowHovered() only: Return true if any children of the
              // window is hovered
  ...
  ImGuiHoveredFlags_AllowWhenBlockedByPopup =
      1 << 5, // Return true even if a popup window is normally blocking access
              // to this item/window
  // ImGuiHoveredFlags_AllowWhenBlockedByModal     = 1 << 6,   // Return true
  ...
  // may to use ImGuiHoveredFlags_ForTooltip (stationary + delay) so the tooltip
  ...
  ImGuiHoveredFlags_ForTooltip =
      1 << 12, // Shortcut for standard flags when using IsItemHovered() +
               // SetTooltip() sequence.

  // (Advanced) Mouse Hovering delays.
  // - generally you can use ImGuiHoveredFlags_ForTooltip to use
...
enum ImGuiDragDropFlags_ {
  ...
  ImGuiDragDropFlags_AcceptPeekOnly =
      ImGuiDragDropFlags_AcceptBeforeDelivery |
      ImGuiDragDropFlags_AcceptNoDrawDefaultRect, // For peeking ahead and
                                                  // inspecting the payload
                                                  // before delivery.
};

// Standard Drag and Drop payload types. You can define you own payload types
// using short strings. Types starting with '_' are defined by Dear ImGui.
...
// need a cast to ImGuiKey. Read details about the 1.87 and 1.89 transition :
...
enum ImGuiKey : int {
  ...
  ImGuiKey_GamepadStart,    // QuickMenu (Xbox)      + (Switch)   Start/Options
                            // (PS)
  ImGuiKey_GamepadBack,     // View (Xbox)      - (Switch)   Share (PS)
  ImGuiKey_GamepadFaceLeft, // X (Xbox)         Y (Switch)   Square (PS) // Tap:
                            // Toggle QuickMenu. Hold: Windowing mode
  ...
  ImGuiMod_Alt = 1 << 14,      // Option/QuickMenu
...
#else
  ImGuiKey_KeysData_SIZE = ImGuiKey_COUNT, // Size of KeysData[]: hold legacy
                                           // 0..512 keycodes + named keys
  ImGuiKey_KeysData_OFFSET = 0, // Accesses to io.KeysData[] must use (key -
                                // ImGuiKey_KeysData_OFFSET) index.
#endif
...
// inputs as long as IMGUI_DISABLE_OBSOLETE_KEYIO is not set. Custom backends:
// feed gamepad inputs via io.AddKeyEvent() and ImGuiKey_GamepadXXX enums.
enum ImGuiNavInput {
...
enum ImGuiConfigFlags_ {
  ...
  ImGuiConfigFlags_NavEnableGamepad =
      1 << 1, // Master gamepad navigation enable flag. Backend also needs to
              // set ImGuiBackendFlags_HasGamepad.
  ImGuiConfigFlags_NavEnableSetMousePos =
      1 << 2, // Instruct navigation to move the mouse cursor. May be useful on
              ...
              // backend, otherwise ImGui will react as if the mouse is jumping
...
// Backend capabilities flags stored in io.BackendFlags. Set by imgui_impl_xxx
// or custom backend.
enum ImGuiBackendFlags_ {
  ImGuiBackendFlags_None = 0,
  ImGuiBackendFlags_HasGamepad = 1 << 0, // Backend Platform supports gamepad
  ...
  ImGuiBackendFlags_RendererHasVtxOffset =
      1 << 3, // Backend Renderer supports ImDrawCmd::VtxOffset. This enables
...
enum ImGuiCol_ {
  ...
  ImGuiCol_FrameBg, // Background of checkbox, radio button, plot, slider, text
                    // input
  ...
  ImGuiCol_Header, // Header* colors are used for CollapsingHeader, TreeNode,
                   // Selectable, MenuItem
  ...
  ImGuiCol_ResizeGrip, // Resize grip in lower-right and lower-left corners of
                       // windows.
  ...
  ImGuiCol_TableBorderStrong, // Table outer and header borders (prefer using
                              // Alpha=1.0 here)
  ImGuiCol_TableBorderLight,  // Table inner borders (prefer using Alpha=1.0
                              // here)
...
// - The enum only refers to fields of ImGuiStyle which makes sense to be
...
// offset/type.
enum ImGuiStyleVar_ {
  // Enum name --------------------- // Member in ImGuiStyle structure (see
  // ImGuiStyle for descriptions)
...
enum ImGuiColorEditFlags_ {
  ...
  ImGuiColorEditFlags_NoPicker =
      1 << 2, //              // ColorEdit: disable picker when clicking on
              //              color square.
  ...
  ImGuiColorEditFlags_HDR =
      1 << 19, //              // (WIP) ColorEdit: Currently only disable
               ...
               //              probably want to use ImGuiColorEditFlags_Float
               //              flag as well).
  ...
  ImGuiColorEditFlags_PickerHueBar = 1
                                     << 25, // [Picker]     // ColorPicker: bar
                                            // for Hue, rectangle for Sat/Value.
  ImGuiColorEditFlags_PickerHueWheel =
      1 << 26, // [Picker]     // ColorPicker: wheel for Hue, triangle for
               // Sat/Value.
  ImGuiColorEditFlags_InputRGB =
      1 << 27, // [Input]      // ColorEdit, ColorPicker: input and output data
               // in RGB format.
  ImGuiColorEditFlags_InputHSV =
      1 << 28, // [Input]      // ColorEdit, ColorPicker: input and output data
               // in HSV format.
...
// per-item flags. There are shared flags in ImGuiIO:
...
enum ImGuiSliderFlags_ {
  ...
  ImGuiSliderFlags_NoRoundToFormat =
      1 << 6, // Disable rounding underlying value to match precision of the
              ...
              // digits)
  ImGuiSliderFlags_NoInput = 1 << 7, // Disable CTRL+Click or Enter key allowing
  ...
  // Obsolete names
  // ImGuiSliderFlags_ClampOnInput = ImGuiSliderFlags_AlwaysClamp, // [renamed
  // in 1.79]
};
...
enum ImGuiMouseCursor_ {
  ...
  ImGuiMouseCursor_ResizeNESW, // When hovering over the bottom-left corner of a
                               // window
  ImGuiMouseCursor_ResizeNWSE, // When hovering over the bottom-right corner of
                               // a window
  ImGuiMouseCursor_Hand,       // (Unused by Dear ImGui functions. Use for e.g.
                               // hyperlinks)
  ImGuiMouseCursor_NotAllowed, // When hovering something with disallowed
...
enum ImGuiMouseSource : int {
  ...
  ImGuiMouseSource_TouchScreen, // Input is coming from a touch screen (no
                                // hovering prior to initial press, less precise
                                // initial press aiming, dual-axis wheeling
                                // possible).
  ImGuiMouseSource_Pen, // Input is coming from a pressure/magnetic pen (often
                        // used in conjunction with high-sampling rates).
  ImGuiMouseSource_COUNT
};

// Enumeration for ImGui::SetWindow***(), SetNextWindow***(), SetNextItem***()
...
// 0 as a shortcut to ImGuiCond_Always.
enum ImGuiCond_ {
  ImGuiCond_None = 0, // No condition (always set the variable), same as _Always
  ImGuiCond_Always =
      1 << 0, // No condition (always set the variable), same as _None
  ImGuiCond_Once = 1 << 1, // Set the variable once per runtime session (only
                           // the first call will succeed)
  ImGuiCond_FirstUseEver =
      1 << 2, // Set the variable if the object/window has no persistently saved
              // data (no entry in .ini file)
  ImGuiCond_Appearing =
      1 << 3, // Set the variable if the object/window is appearing after being
...
// [SECTION] ImGuiStyle
//-----------------------------------------------------------------------------
// You may modify the ImGui::GetStyle() main instance during initialization and
// before NewFrame(). During the frame, use
// ImGui::PushStyleVar(ImGuiStyleVar_XXXX)/PopStyleVar() to alter the main style
// values, and ImGui::PushStyleColor(ImGuiCol_XXX)/PopStyleColor() for colors.
//-----------------------------------------------------------------------------
...
struct ImGuiStyle {
  ...
  ImVec2 WindowMinSize; // Minimum window size. This is a global setting. If you
                        // want to constrain individual windows, use
                        // SetNextWindowSizeConstraints().
  ImVec2 WindowTitleAlign; // Alignment for title bar text. Defaults to
                           // (0.0f,0.5f) for left-aligned,vertically centered.
  ImGuiDir WindowMenuButtonPosition; // Side of the collapsing/docking button in
  ...
  ImVec2 ItemInnerSpacing;  // Horizontal and vertical spacing between within
                            // elements of a composed widget (e.g. a slider and
                            // its label).
  ImVec2 CellPadding;       // Padding within a table cell. CellPadding.y may be
                            // altered between different rows.
  ImVec2 TouchExtraPadding; // Expand reactive bounding box for touch-based
  ...
  ImVec2 ButtonTextAlign; // Alignment of button text when button is larger than
                          // text. Defaults to (0.5f, 0.5f) (centered).
  ImVec2 SelectableTextAlign; // Alignment of selectable text. Defaults to
  ...
  ImVec2 SeparatorTextAlign; // Alignment of text within the separator. Defaults
                             // to (0.0f, 0.5f) (left aligned, center).
  ImVec2 SeparatorTextPadding; // Horizontal offset of text from each edge of
                               // the separator + spacing on other axis.
                               // Generally small values. .y is recommended to
                               // be == FramePadding.y.
  ImVec2
      DisplayWindowPadding; // Window position are clamped to be visible within
                            // the display area or monitors by at least this
                            // amount. Only applies to regular windows.
  ImVec2 DisplaySafeAreaPadding; // If you cannot see the edges of your screen
  ...
  bool AntiAliasedFill;        // Enable anti-aliased edges around filled shapes
                        ...
                        // of the frame (copied to ImDrawList).
  ...
  float HoverStationaryDelay; // Delay for
                              // IsItemHovered(ImGuiHoveredFlags_Stationary).
                              // Time required to consider mouse stationary.
  float
      HoverDelayShort; // Delay for IsItemHovered(ImGuiHoveredFlags_DelayShort).
  ...
  ImGuiHoveredFlags
      HoverFlagsForTooltipNav; // Default flags when using
                               // IsItemHovered(ImGuiHoveredFlags_ForTooltip) or
...
// [SECTION] ImGuiIO
//-----------------------------------------------------------------------------
// Communicate most settings and inputs/outputs to Dear ImGui using this
// structure. Access via ImGui::GetIO(). Read 'Programmer guide' section in .cpp
...
struct ImGuiIO {
  ...
  ImGuiConfigFlags ConfigFlags; // = 0              // See ImGuiConfigFlags_
                                // enum. Set by user/application.
                                // Gamepad/keyboard navigation options, etc.
  ImGuiBackendFlags
      BackendFlags;   // = 0              // See ImGuiBackendFlags_ enum. Set by
  ...
  const char
      *IniFilename; // = "imgui.ini"    // Path to .ini file (important: default
                    // "imgui.ini" is relative to current working dir!). Set
  ...
  const char
      *LogFilename; // = "imgui_log.txt"// Path to .log file (default parameter
                    // to ImGui::LogToFile when no file is specified).
  ...
  ImFont *FontDefault; // = NULL           // Font to use on NewFrame(). Use
                       // NULL to uses Fonts->Fonts[0].
  ImVec2 DisplayFramebufferScale; // = (1, 1)         // For retina display or
  ...
  bool MouseDrawCursor; // = false          // Request ImGui to draw a mouse
  ...
  // ImOsIsDebuggerPresent() in imgui_test_engine/imgui_te_utils.cpp for a Unix
  // compatible version).
  ...
  ImWchar PlatformLocaleDecimalPoint; // '.'              // [Experimental]
  ...
  IMGUI_API void AddKeyEvent(
      ImGuiKey key,
      bool down); // Queue a new key down/up event. Key should be "translated"
                  // (as in, generally ImGuiKey_A matches the key end-user would
                  // use to emit an 'A' character)
  IMGUI_API void
  AddKeyAnalogEvent(ImGuiKey key, bool down,
                    float v); // Queue a new key down/up event for analog values
                              // (e.g. ImGuiKey_Gamepad_ values). Dead-zones
  ...
  IMGUI_API void AddMouseButtonEvent(int button,
  ...
  IMGUI_API void
  AddMouseSourceEvent(ImGuiMouseSource source); // Queue a mouse source change
  ...
  IMGUI_API void
  AddInputCharacter(unsigned int c); // Queue a new character input
  IMGUI_API void AddInputCharacterUTF16(
      ImWchar16 c); // Queue a new character input from a UTF-16 character, it
                    // can be a surrogate
  ...
  IMGUI_API void ClearInputKeys(); // Clear current keyboard/mouse/gamepad state
                                   // + current frame text input buffer.
                                   // Equivalent to releasing all keys/buttons.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  IMGUI_API void
  ClearInputCharacters(); // [Obsolete] Clear the current frame text input
                          // buffer. Now included within ClearInputKeys().
#endif
  ...
  bool WantCaptureMouse; // Set when Dear ImGui will use mouse inputs, in this
  ...
  bool WantCaptureKeyboard; // Set when Dear ImGui will use keyboard inputs, in
  ...
  bool WantTextInput; // Mobile/console: when set, you may display an on-screen
                      // keyboard. This is set by Dear ImGui when it wants
  ...
  //   Old (<1.87):  ImGui::IsKeyPressed(ImGui::GetIO().KeyMap[ImGuiKey_Space])
  //   --> New (1.87+) ImGui::IsKeyPressed(ImGuiKey_Space)
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
  int KeyMap[ImGuiKey_COUNT]; // [LEGACY] Input: map of indices into the
                              ...
                              // ImGuiKey_ indices which are always >512.
  bool KeysDown[ImGuiKey_COUNT]; // [LEGACY] Input: Keyboard keys that are
                                 ...
                                 // now ImGuiKey_COUNT to allow legacy
                                 // io.KeysDown[GetKeyIndex(...)] to work
                                 // without an overflow.
  float NavInputs[ImGuiNavInput_COUNT]; // [LEGACY] Since 1.88, NavInputs[] was
                                        // removed. Backends from 1.60 to 1.86
                                        // won't build. Feed gamepad inputs via
                                        // io.AddKeyEvent() and
                                        // ImGuiKey_GamepadXXX enums.
#endif
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
  void
      *ImeWindowHandle; // = NULL   // [Obsoleted in 1.87] Set
                        // ImGuiViewport::PlatformHandleRaw instead. Set this to
  ...
  //------------------------------------------------------------------
  // [Internal] Dear ImGui will maintain those fields. Forward compatibility not
  // guaranteed!
  //------------------------------------------------------------------

  ImGuiContext
      *Ctx; // Parent UI context (needs to be set explicitly by parent).
  ...
  // moving anywhere)
  ImVec2
      MousePos; // Mouse position, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX)
  ...
  ImGuiKeyChord
      KeyMods; // Key mods flags (any of
               // ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super
               // flags, same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged
               // into flags. DOES NOT CONTAINS ImGuiMod_Shortcut which is
               // pretranslated). Read-only, updated by NewFrame()
  ImGuiKeyData
      KeysData[ImGuiKey_KeysData_SIZE];  // Key state for all known keys. Use
  ...
  ImVec2 MousePosPrev; // Previous mouse position (note that MouseDelta is not
  ...
  ImU16 MouseClickedCount[5]; // == 0 (not clicked), == 1 (same as
                              // MouseClicked[]), == 2 (double-clicked), == 3
                              // (triple-clicked) etc. when going from !Down to
                              // Down
  ImU16 MouseClickedLastCount[5]; // Count successive number of clicks. Stays
  ...
  float PenPressure; // Touch/Pen pressure (0.0f to 1.0f, should be >0.0f only
                     ...
                     // unused by Dear ImGui.
  bool AppFocusLost; // Only modify via AddFocusEvent()
  bool AppAcceptingEvents;          // Only modify via SetAppAcceptingEvents()
  ImS8 BackendUsingLegacyKeyArrays; // -1: unknown, 0: using AddKeyEvent(), 1:
...
// ImGuiInputTextFlags_Callback* flag is used. The callback function should
// return 0 by default. Callbacks (follow a flag name and see comments in
// ImGuiInputTextFlags_ declarations for more details)
// - ImGuiInputTextFlags_CallbackEdit:        Callback on buffer edit (note that
...
// - ImGuiInputTextFlags_CallbackHistory:     Callback on pressing Up/Down
// arrows
// - ImGuiInputTextFlags_CallbackCharFilter:  Callback on character inputs to
...
struct ImGuiInputTextCallbackData {
  ...
  ImGuiInputTextFlags
      EventFlag; // One ImGuiInputTextFlags_Callback*    // Read-only
  ImGuiInputTextFlags
      Flags;      // What user passed to InputText()      // Read-only
  ...
  IMGUI_API void InsertChars(int pos, const char *text,
...
struct ImGuiTableColumnSortSpecs {
  ImGuiID ColumnUserID; // User id of the column (if specified by a
                        // TableSetupColumn() call)
  ImS16 ColumnIndex;    // Index of the column
  ImS16 SortOrder; // Index within parent ImGuiTableSortSpecs (always stored in
                   // order starting from 0, tables sorted on a single criteria
                   // will always have a 0 here)
  ImGuiSortDirection SortDirection : 8; // ImGuiSortDirection_Ascending or
...
struct ImGuiTableSortSpecs {
  ...
  int SpecsCount;  // Sort spec count. Most often 1. May be > 1 when
                   // ImGuiTableFlags_SortMulti is enabled. May be == 0 when
                   // ImGuiTableFlags_SortTristate is enabled.
...
// every frame. Usage: static ImGuiOnceUponAFrame oaf; if (oaf)
...
struct ImGuiStorage {
  ...
  // struct)
  //      float* pvar = ImGui::GetFloatRef(key); ImGui::SliderFloat("var", pvar,
...
// ImGui already clip items based on their bounds but: it needs to first layout
// the item to do so, and generally
//  fetching/submitting your own data incurs additional cost. Coarse clipping
//  using ImGuiListClipper allows you to easily scale using lists with tens of
...
// ImVec2/ImVec4 and their own types (using IM_VEC2_CLASS_EXTRA in imconfig.h)
// - You can use '#define IMGUI_DEFINE_MATH_OPERATORS' to import our operators,
...
// Helper: ImColor() implicitly converts colors to either ImU32 (packed 4x1
// byte) or ImVec4 (4x1 float) Prefer using IM_COL32() macros if you want a
// guaranteed compile-time ImU32 for usage with ImDrawList API.
...
// convenience to pass colors in either ImU32 or ImVec4 formats. Explicitly cast
// to ImU32 or ImVec4 if needed.
struct ImColor {
  ImVec4 Value;
...
//-----------------------------------------------------------------------------
// [SECTION] Drawing API (ImDrawCmd, ImDrawIdx, ImDrawVert, ImDrawChannel,
// ImDrawListSplitter, ImDrawListFlags, ImDrawList, ImDrawData) Hold a series of
// drawing commands. The user provides a renderer for ImDrawData which
// essentially contains an array of ImDrawList.
//-----------------------------------------------------------------------------

// The maximum line width to bake anti-aliased textures for. Build atlas with
// ImFontAtlasFlags_NoBakedLines to disable baking.
#ifndef IM_DRAWLIST_TEX_LINES_WIDTH_MAX
#define IM_DRAWLIST_TEX_LINES_WIDTH_MAX (63)
#endif

// ImDrawCallback: Draw callbacks for advanced uses [configurable type: override
// in imconfig.h] NB: You most likely do NOT need to use draw callbacks just to
...
// If you want to override the signature of ImDrawCallback, you can simply use
// e.g. '#define ImDrawCallback MyDrawCallback' (in imconfig.h) + update
...
// altered the render state and you want it to be restored. It is not done by
// default because they are many perfectly useful way of altering render state
// for imgui contents (e.g. changing shader/blending settings before an Image
// call).
...
struct ImDrawCmd {
  ImVec4 ClipRect; // 4*4  // Clipping rectangle (x1, y1, x2, y2). Subtract
                   // ImDrawData->DisplayPos to get clipping rectangle in
                   // "viewport" coordinates
  ImTextureID TextureId;  // 4-8  // User-provided texture ID. Set by user in
  ...
  unsigned int VtxOffset; // 4    // Start offset in vertex buffer.
                          // ImGuiBackendFlags_RendererHasVtxOffset: always 0,
  ...
  ImDrawCallback UserCallback; // 4-8  // If != NULL, call the function instead
  ...
  // Since 1.83: returns ImTextureID associated with this draw call. Warning: DO
...
// IMGUI_OVERRIDE_DRAWVERT_STRUCT_LAYOUT in imconfig.h The code expect ImVec2
// pos (8 bytes), ImVec2 uv (8 bytes), ImU32 col (4 bytes), but you can re-order
...
// struct or use a typedef). This is because ImVec2/ImU32 are likely not
// declared at the time you'd want to set your type up. NOTE: IMGUI DOESN'T
...
struct ImDrawListSplitter {
  ...
  ImVector<ImDrawChannel> _Channels; // Draw channels (not resized down so
...
// (Legacy: bit 0 must always correspond to ImDrawFlags_Closed to be backward
...
enum ImDrawFlags_ {
  ...
  ImDrawFlags_RoundCornersDefault_ =
      ImDrawFlags_RoundCornersAll, // Default to ALL corners if none of the
...
// functions from ImGuiIO settings, and generally not manipulated directly. It
// is however possible to temporarily alter flags between calls to ImDrawList::
// functions.
...
// contains its own ImDrawList. You can use ImGui::GetWindowDrawList() to access
// the current window draw list and draw custom primitives. You can interleave
// normal ImGui:: calls and adding primitives to the current draw list. In
...
// culled (culling is done at higher-level by ImGui:: functions), if you use
// this API a lot consider coarse culling your drawn objects.
struct ImDrawList {
  // This is what you have to render
  ImVector<ImDrawCmd> CmdBuffer; // Draw commands. Typically 1 command = 1 GPU
                                 // draw call, unless the command is a callback.
  ImVector<ImDrawIdx> IdxBuffer; // Index buffer. Each command consume
                                 // ImDrawCmd::ElemCount of those
  ImVector<ImDrawVert> VtxBuffer; // Vertex buffer.
  ImDrawListFlags Flags;          // Flags, you may poke into these to adjust
  ...
  ImDrawListSharedData *_Data; // Pointer to shared draw data (you can use
                               // ImGui::GetDrawListSharedData() to get the one
                               // from current ImGui context)
  const char *_OwnerName;      // Pointer to owner window's name for debugging
  ImDrawVert *_VtxWritePtr; // [Internal] point within VtxBuffer.Data after each
  ...
  ImDrawCmdHeader _CmdHeader; // [Internal] template of active commands. Fields
  ...
  // If you want to create ImDrawList instances, pass them
  // ImGui::GetDrawListSharedData() or create and use your own
  ...
  //   sides.
  IMGUI_API void AddLine(const ImVec2 &p1, const ImVec2 &p2, ImU32 col,
                         float thickness = 1.0f);
  IMGUI_API void
  AddRect(const ImVec2 &p_min, const ImVec2 &p_max, ImU32 col,
          float rounding = 0.0f, ImDrawFlags flags = 0,
          float thickness =
              1.0f); // a: upper-left, b: lower-right (== upper-left + size)
  IMGUI_API void
  AddRectFilled(const ImVec2 &p_min, const ImVec2 &p_max, ImU32 col,
                float rounding = 0.0f,
                ImDrawFlags flags =
                    0); // a: upper-left, b: lower-right (== upper-left + size)
  IMGUI_API void
  AddRectFilledMultiColor(const ImVec2 &p_min, const ImVec2 &p_max,
                          ImU32 col_upr_left, ImU32 col_upr_right,
                          ImU32 col_bot_right, ImU32 col_bot_left);
  IMGUI_API void AddQuad(const ImVec2 &p1, const ImVec2 &p2, const ImVec2 &p3,
                         const ImVec2 &p4, ImU32 col, float thickness = 1.0f);
  IMGUI_API void AddQuadFilled(const ImVec2 &p1, const ImVec2 &p2,
                               const ImVec2 &p3, const ImVec2 &p4, ImU32 col);
  IMGUI_API void AddTriangle(const ImVec2 &p1, const ImVec2 &p2,
                             const ImVec2 &p3, ImU32 col,
                             float thickness = 1.0f);
  IMGUI_API void AddTriangleFilled(const ImVec2 &p1, const ImVec2 &p2,
                                   const ImVec2 &p3, ImU32 col);
  IMGUI_API void AddCircle(const ImVec2 &center, float radius, ImU32 col,
                           int num_segments = 0, float thickness = 1.0f);
  IMGUI_API void AddCircleFilled(const ImVec2 &center, float radius, ImU32 col,
                                 int num_segments = 0);
  IMGUI_API void AddNgon(const ImVec2 &center, float radius, ImU32 col,
                         int num_segments, float thickness = 1.0f);
  IMGUI_API void AddNgonFilled(const ImVec2 &center, float radius, ImU32 col,
                               int num_segments);
  IMGUI_API void AddEllipse(const ImVec2 &center, float radius_x,
                            float radius_y, ImU32 col, float rot = 0.0f,
                            int num_segments = 0, float thickness = 1.0f);
  IMGUI_API void AddEllipseFilled(const ImVec2 &center, float radius_x,
                                  float radius_y, ImU32 col, float rot = 0.0f,
                                  int num_segments = 0);
  IMGUI_API void AddText(const ImVec2 &pos, ImU32 col, const char *text_begin,
                         const char *text_end = NULL);
  ...
  IMGUI_API void AddImage(ImTextureID user_texture_id, const ImVec2 &p_min,
                          const ImVec2 &p_max,
                          const ImVec2 &uv_min = ImVec2(0, 0),
                          const ImVec2 &uv_max = ImVec2(1, 1),
                          ImU32 col = IM_COL32_WHITE);
  IMGUI_API void AddImageQuad(
      ImTextureID user_texture_id, const ImVec2 &p1, const ImVec2 &p2,
      const ImVec2 &p3, const ImVec2 &p4, const ImVec2 &uv1 = ImVec2(0, 0),
      const ImVec2 &uv2 = ImVec2(1, 0), const ImVec2 &uv3 = ImVec2(1, 1),
      const ImVec2 &uv4 = ImVec2(0, 1), ImU32 col = IM_COL32_WHITE);
  ...
  inline void PathStroke(ImU32 col, ImDrawFlags flags = 0,
                         float thickness = 1.0f) {
    AddPolyline(_Path.Data, _Path.Size, col, flags, thickness);
    _Path.Size = 0;
  }
  IMGUI_API void PathArcTo(const ImVec2 &center, float radius, float a_min,
                           float a_max, int num_segments = 0);
  IMGUI_API void PathArcToFast(
      const ImVec2 &center, float radius, int a_min_of_12,
      int a_max_of_12); // Use precomputed angles for a 12 steps circle
  IMGUI_API void PathEllipticalArcTo(const ImVec2 &center, float radius_x,
                                     float radius_y, float rot, float a_min,
                                     float a_max,
                                     int num_segments = 0); // Ellipse
  IMGUI_API void PathBezierCubicCurveTo(
      const ImVec2 &p2, const ImVec2 &p3, const ImVec2 &p4,
      int num_segments = 0); // Cubic Bezier (4 control points)
  IMGUI_API void PathBezierQuadraticCurveTo(
      const ImVec2 &p2, const ImVec2 &p3,
      int num_segments = 0); // Quadratic Bezier (3 control points)
  IMGUI_API void PathRect(const ImVec2 &rect_min, const ImVec2 &rect_max,
                          float rounding = 0.0f, ImDrawFlags flags = 0);

  // Advanced
  IMGUI_API void
  AddCallback(ImDrawCallback callback,
              void *callback_data); // Your rendering function must check for
                                    // 'UserCallback' in ImDrawCmd and call the
  ...
  //   Prefer using your own persistent instance of ImDrawListSplitter as you
  ...
  IMGUI_API void
  PrimRect(const ImVec2 &a, const ImVec2 &b,
           ImU32 col); // Axis aligned rectangle (composed of two triangles)
  IMGUI_API void PrimRectUV(const ImVec2 &a, const ImVec2 &b,
                            const ImVec2 &uv_a, const ImVec2 &uv_b, ImU32 col);
  IMGUI_API void PrimQuadUV(const ImVec2 &a, const ImVec2 &b, const ImVec2 &c,
                            const ImVec2 &d, const ImVec2 &uv_a,
                            const ImVec2 &uv_b, const ImVec2 &uv_c,
  ...
  // Obsolete names
  // inline  void  AddBezierCurve(const ImVec2& p1, const ImVec2& p2, const
  // ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness, int num_segments
  // = 0) { AddBezierCubic(p1, p2, p3, p4, col, thickness, num_segments); } //
  // OBSOLETED in 1.80 (Jan 2021) inline  void  PathBezierCurveTo(const ImVec2&
  // p2, const ImVec2& p3, const ImVec2& p4, int num_segments = 0) {
  ...
  IMGUI_API void _PathArcToFastEx(const ImVec2 &center, float radius,
                                  int a_min_sample, int a_max_sample,
                                  int a_step);
  IMGUI_API void _PathArcToN(const ImVec2 &center, float radius, float a_min,
...
// the oldest structure exposed by the library! Basically, ImDrawList ==
// CmdList)
struct ImDrawData {
  bool Valid;        // Only valid after Render() is called and before the next
                     // NewFrame() is called.
  int CmdListsCount; // Number of ImDrawList* to render (should always be ==
                     // CmdLists.size)
  int TotalIdxCount; // For convenience, sum of all ImDrawList's IdxBuffer.Size
  int TotalVtxCount; // For convenience, sum of all ImDrawList's VtxBuffer.Size
  ImVector<ImDrawList *>
      CmdLists; // Array of ImDrawList* to render. The ImDrawLists are owned by
                // ImGuiContext and only pointed to from here.
  ImVec2 DisplayPos; // Top-left position of the viewport to render (== top-left
  ...
  ImVec2 DisplaySize; // Size of the viewport to render (==
                      // GetMainViewport()->Size for the main viewport, ==
                      // io.DisplaySize in most single-viewport applications)
  ImVec2
      FramebufferScale; // Amount of pixels for each unit of DisplaySize. Based
                        // on io.DisplayFramebufferScale. Generally (1,1) on
                        // normal display, (2,2) on OSX with Retina display.
  ImGuiViewport
      *OwnerViewport; // Viewport carrying the ImDrawData instance, might be of
                      // use to the renderer (generally not).

  // Functions
  ImDrawData() { Clear(); }
  IMGUI_API void Clear();
  IMGUI_API void
  AddDrawList(ImDrawList *draw_list); // Helper to add an external draw list
                                      // into an existing ImDrawData.
  IMGUI_API void
  ...
  IMGUI_API void ScaleClipRects(
      const ImVec2 &fb_scale); // Helper to scale the ClipRect field of each
                               // ImDrawCmd. Use if your final output buffer is
                               // at a different scale than Dear ImGui expects,
                               // or if there is a difference between your
                               // window resolution and framebuffer resolution.
};

//-----------------------------------------------------------------------------
// [SECTION] Font API (ImFontConfig, ImFontGlyph, ImFontAtlasFlags, ImFontAtlas,
...
struct ImFontConfig {
  ...
  bool FontDataOwnedByAtlas; // true     // TTF/OTF data ownership taken by the
                             // container ImFontAtlas (will delete memory
                             // itself).
  ...
  ImVec2 GlyphExtraSpacing; // 0, 0     // Extra spacing (in pixels) between
  ...
  bool MergeMode; // false    // Merge into previous ImFont, so you can combine
  ...
  float RasterizerMultiply; // 1.0f     // Brighten (>1.0f) or darken (<1.0f)
                            // font output. Brightening small fonts may be a
                            // good workaround to make them more readable.
  ImWchar EllipsisChar; // -1       // Explicitly specify unicode codepoint of
...
struct ImFontAtlasCustomRect {
  ...
  ImVec2 GlyphOffset; // Input    // For custom font glyphs only: glyph display
                      // offset
...
//    texture. Read FAQ entry about ImTextureID for more details.
// Common pitfalls:
...
struct ImFontAtlas {
  ...
  IMGUI_API ImFont *AddFontFromFileTTF(const char *filename, float size_pixels,
  ...
  IMGUI_API ImFont *AddFontFromMemoryTTF(
      ...
      const ImWchar *glyph_ranges =
          NULL); // Note: Transfer ownership of 'ttf_data' to ImFontAtlas! Will
  ...
  IMGUI_API ImFont *AddFontFromMemoryCompressedBase85TTF(
      ...
      const ImWchar *glyph_ranges =
          NULL); // 'compressed_font_data_base85' still owned by caller.
                 ...
                 // parameter.
  IMGUI_API void
  ClearInputData(); // Clear input data (all ImFontConfig structures including
  ...
  IMGUI_API bool Build(); // Build pixels data. This is called automatically for
  ...
  // for details. NB: Consider using ImFontGlyphRangesBuilder to build glyph
  ...
  IMGUI_API int AddCustomRectFontGlyph(ImFont *font, ImWchar id, int width,
  ...
  // [Internal]
  IMGUI_API void CalcCustomRectUV(const ImFontAtlasCustomRect *rect,
                                  ImVec2 *out_uv_min, ImVec2 *out_uv_max) const;
  ...
  ImTextureID TexID;      // User data to refer to the texture once it has been
                     // uploaded to user's graphic systems. It is passed back to
                     // you during rendering via the ImDrawCmd structure.
  ...
  bool Locked; // Marked as Locked by ImGui::NewFrame() so attempt to modify the
               // atlas will assert.
  ...
  ImVector<ImFont *>
      Fonts; // Hold all the fonts returned by AddFont*. Fonts[0] is the default
  ...
  ImVec4 TexUvLines[IM_DRAWLIST_TEX_LINES_WIDTH_MAX +
  ...
  const ImFontBuilderIO
      *FontBuilderIO; // Opaque interface to a font builder (default to
                      ...
                      // defining IMGUI_ENABLE_FREETYPE).
  unsigned int
...
struct ImFont {
  ...
  ImVector<ImWchar> IndexLookup; // 12-16 // out //            // Sparse. Index
  ...
  ImFontAtlas *ContainerAtlas; // 4-8   // out //            // What we has been
                               // loaded into
  const ImFontConfig *ConfigData; // 4-8   // in  //            // Pointer
                                  // within ContainerAtlas->ConfigData
  short ConfigDataCount; // 2     // in  // ~ 1        // Number of ImFontConfig
                         // involved in creating this font. Bigger than 1 when
                         // merging multiple font sources into one ImFont.
  ImWchar FallbackChar;  // 2     // out // = FFFD/'?' // Character used if a
                         // glyph isn't found.
  ImWchar EllipsisChar;  // 2     // out // = '...'/'.'// Character used for
                         // ellipsis rendering.
  ...
  ImU8 Used4kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 4096 /
                      8]; // 2 bytes if ImWchar=ImWchar16, 34 bytes if
                          // ImWchar==ImWchar32. Store 1-bit for each block of
  ...
  IMGUI_API ImVec2 CalcTextSizeA(float size, float max_width, float wrap_width,
                                 const char *text_begin,
                                 const char *text_end = NULL,
                                 const char **remaining = NULL) const; // utf8
  IMGUI_API const char *CalcWordWrapPositionA(float scale, const char *text,
                                              const char *text_end,
                                              float wrap_width) const;
  IMGUI_API void RenderChar(ImDrawList *draw_list, float size,
                            const ImVec2 &pos, ImU32 col, ImWchar c) const;
  IMGUI_API void RenderText(ImDrawList *draw_list, float size,
                            const ImVec2 &pos, ImU32 col,
                            const ImVec4 &clip_rect, const char *text_begin,
  ...
  IMGUI_API void AddGlyph(const ImFontConfig *src_cfg, ImWchar c, float x0,
...
// Flags stored in ImGuiViewport::Flags, giving indications to the platform
// backends.
...
struct ImGuiViewport {
  ...
  ImVec2 Pos; // Main Area: Position of the viewport (Dear ImGui coordinates are
              // the same as OS desktop/native coordinates)
  ImVec2 Size;     // Main Area: Size of the viewport.
  ImVec2 WorkPos;  // Work Area: Position of the viewport minus task bars, menus
...
//-----------------------------------------------------------------------------
// [SECTION] Obsolete functions and types
// (Will be removed! Read 'API BREAKING CHANGES' section in imgui.cpp for
// details) Please keep your copy of dear imgui up to date! Occasionally set
// '#define IMGUI_DISABLE_OBSOLETE_FUNCTIONS' in imconfig.h to stay ahead.
//-----------------------------------------------------------------------------

namespace ImGui {
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
IMGUI_API ImGuiKey
GetKeyIndex(ImGuiKey key); // map ImGuiKey_* values into legacy native key
                           // index. == io.KeyMap[key]
#else
static inline ImGuiKey GetKeyIndex(ImGuiKey key) {
  IM_ASSERT(
      ...
      "disabled by IMGUI_DISABLE_OBSOLETE_KEYIO. Please switch to ImGuiKey.");
  return key;
}
#endif
} // namespace ImGui
...
IMGUI_API bool
ImageButton(ImTextureID user_texture_id, const ImVec2 &size,
            const ImVec2 &uv0 = ImVec2(0, 0), const ImVec2 &uv1 = ImVec2(1, 1),
            int frame_padding = -1, const ImVec4 &bg_col = ImVec4(0, 0, 0, 0),
            const ImVec4 &tint_col =
                ImVec4(1, 1, 1, 1)); // Use new ImageButton() signature
...
IMGUI_API void CalcListClipping(
    ...
    int *out_items_display_end); // Calculate coarse clipping for large list of
                                 // evenly sized items. Prefer using
                                 // ImGuiListClipper.
...
// static inline bool  ListBoxHeader(const char* label, const ImVec2& size =
// ImVec2(0, 0))         { return BeginListBox(label, size); } static inline
...
// ImMin(items_count, 7) : height_in_items) + 0.25f) + GetStyle().FramePadding.y
// * 2.0f; return BeginListBox(label, ImVec2(0.0f, height)); } // Helper to
...
// ImGuiMouseButton mb = 1)    { OpenPopupOnItemClick(str_id, mb); } // Bool
...
// power = 1.0f)                                                            //
// OBSOLETED in 1.78 (from June 2020) IMGUI_API bool      DragScalarN(const
// char* label, ImGuiDataType data_type, void* p_data, int components, float
// v_speed, const void* p_min, const void* p_max, const char* format, float
// power = 1.0f);                                          // OBSOLETED in 1.78
// (from June 2020) IMGUI_API bool      SliderScalar(const char* label,
...
// 2020) IMGUI_API bool      SliderScalarN(const char* label, ImGuiDataType
...
// DragScalarN(label, ImGuiDataType_Float, v, 2, v_speed, &v_min, &v_max,
...
// format, float power = 1.0f) { return DragScalarN(label, ImGuiDataType_Float,
...
// power = 1.0f)              { return SliderScalarN(label, ImGuiDataType_Float,
...
// SliderScalarN(label, ImGuiDataType_Float, v, 4, &v_min, &v_max, format,
...
// ImGuiMouseButton mb, bool over_items) { return
// BeginPopupContextWindow(str_id, mb | (over_items ? 0 :
// ImGuiPopupFlags_NoOpenOverItems)); } // OBSOLETED in 1.77 (from June 2020)
...
// SetNextTreeNodeOpen(bool open, ImGuiCond cond = 0) { SetNextItemOpen(open,
...
// in 1.70 (from May 2019) static inline ImDrawList* GetOverlayDrawList() {
...
// IsWindowFocused(ImGuiFocusedFlags_AnyWindow); }            // OBSOLETED
// in 1.60 (from Apr 2018) static inline bool  IsAnyWindowHovered() { return
// IsWindowHovered(ImGuiHoveredFlags_AnyWindow); }            // OBSOLETED
...
// IsWindowFocused(ImGuiFocusedFlags_RootWindow); }           // OBSOLETED
// in 1.53 (between Oct 2017 and Dec 2017) static inline bool
// IsRootWindowOrAnyChildFocused()       { return
// IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows); }  // OBSOLETED
// in 1.53 (between Oct 2017 and Dec 2017) static inline void
// SetNextWindowContentWidth(float w)    { SetNextWindowContentSize(ImVec2(w,
...
// in 1.53 (between Oct 2017 and Dec 2017) IMGUI_API bool      Begin(char* name,
// bool* p_open, ImVec2 size_first_use, float bg_alpha = -1.0f, ImGuiWindowFlags
// flags=0); // OBSOLETED in 1.52 (between Aug 2017 and Oct 2017): Equivalent of
// using SetNextWindowSize(size, ImGuiCond_FirstUseEver) and
...
// OBSOLETED in 1.52 (between Aug 2017 and Oct 2017) static inline void
// SetNextWindowPosCenter(ImGuiCond c=0) {
// SetNextWindowPos(GetMainViewport()->GetCenter(), c, ImVec2(0.5f,0.5f)); } //
// OBSOLETED in 1.52 (between Aug 2017 and Oct 2017) static inline bool
// IsItemHoveredRect()                   { return
// IsItemHovered(ImGuiHoveredFlags_RectOnly); }               // OBSOLETED
// in 1.51 (between Jun 2017 and Aug 2017) static inline bool
// IsPosHoveringAnyWindow(const ImVec2&) { IM_ASSERT(0); return false; } //
...
// ImGuiHoveredFlags_AllowWhenBlockedByActiveItem); }       // OBSOLETED in 1.51
...
// ImFont*GetWindowFont()                      { return GetFont(); } //
...
//    == 0 prior to 1.82, this is now == ImDrawFlags_RoundCornersNone which is
//    != 0 and not implicit ImDrawCornerFlags_TopLeft   =
//    ImDrawFlags_RoundCornersTopLeft,      // Was == 0x01 (1 << 0) prior
//    to 1.82. Order matches ImDrawFlags_NoRoundCorner* flag (we exploit this
//    internally). ImDrawCornerFlags_TopRight  =
//    ImDrawFlags_RoundCornersTopRight,     // Was == 0x02 (1 << 1) prior
//    to 1.82. ImDrawCornerFlags_BotLeft   = ImDrawFlags_RoundCornersBottomLeft,
//    // Was == 0x04 (1 << 2) prior to 1.82. ImDrawCornerFlags_BotRight  =
//    ImDrawFlags_RoundCornersBottomRight,  // Was == 0x08 (1 << 3) prior
//    to 1.82. ImDrawCornerFlags_All       = ImDrawFlags_RoundCornersAll, // Was
//    == 0x0F prior to 1.82 ImDrawCornerFlags_Top       =
//    ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight,
//    ImDrawCornerFlags_Bot       = ImDrawCornerFlags_BotLeft |
//    ImDrawCornerFlags_BotRight, ImDrawCornerFlags_Left      =
...
// RENAMED and MERGED both ImGuiKey_ModXXX and ImGuiModFlags_XXX into
// ImGuiMod_XXX (from September 2022) RENAMED ImGuiKeyModFlags -> ImGuiModFlags
...
// the first place.
typedef ImGuiKeyChord
    ImGuiModFlags; // == int. We generally use ImGuiKeyChord to mean "a ImGuiKey
                   // or-ed with any number of ImGuiMod_XXX value", but you may
...
// enum ImGuiKeyModFlags_ { ImGuiKeyModFlags_None = 0, ImGuiKeyModFlags_Ctrl =
// ImGuiMod_Ctrl, ImGuiKeyModFlags_Shift = ImGuiMod_Shift, ImGuiKeyModFlags_Alt
// = ImGuiMod_Alt, ImGuiKeyModFlags_Super = ImGuiMod_Super };

#endif // #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS

// RENAMED IMGUI_DISABLE_METRICS_WINDOW > IMGUI_DISABLE_DEBUG_TOOLS in 1.88
// (from June 2022)
...
// Include imgui_user.h at the end of imgui.h (convenient for user to only
// explicitly include vanilla imgui.h)
