...
void ActivityBucketsListModel::refreshData() {
  ...
  if (m_granularity == Day && !m_allSessions.empty()) {
    ...
    const QDate firstDay =
        QDateTime::fromMSecsSinceEpoch(static_cast<qint64>(earliestMs), QTimeZone::systemTimeZone()).date();
    const QDate today = QDate::currentDate();

    for (QDate day = firstDay; day <= today; day = day.addDays(1)) {
      const auto dayStartMs =
          static_cast<uint64_t>(QDateTime(day, QTime(0, 0, 0), QTimeZone::systemTimeZone()).toMSecsSinceEpoch());
      ...
      range.label = QString("%1 %2 %3 (%4)")
                        .arg(QString::number(day.day()), monthString, QString::number(day.year()), dayOfWeekString);
      qint64 dayTotalMs = 0;
      qint64 maxHourMs = 0;
      ...
      for (int hour = 0; hour < 24; ++hour) {
        ...
        for (auto it = playtimeByGame.constBegin(); it != playtimeByGame.constEnd(); ++it) {
          ...
          const auto platform = entry ? getPlatformService()->getPlatform(entry->platformId) : std::nullopt;
          ...
          thing.percentPlayed =
              hourTotalMs > 0 ? static_cast<float>(it.value()) / static_cast<float>(hourTotalMs) : 0.0f;

          bucket.gameActivityRecords.append(thing);
        }

        std::ranges::sort(bucket.gameActivityRecords,
                          [](const Thing &a, const Thing &b) { return a.secondsPlayed > b.secondsPlayed; });

        bucket.numberOfRecords = bucket.gameActivityRecords.size();
        range.buckets.append(bucket);
      }

      for (auto it = totalBucketPlaytimeByGame.constBegin(); it != totalBucketPlaytimeByGame.constEnd(); ++it) {
        const auto entry = getLibraryService()->getEntryWithContentHash(it.key().toStdString());
        const auto displayName = entry ? QString::fromStdString(entry->displayName) : it.key();
        const auto platform = entry ? getPlatformService()->getPlatform(entry->platformId) : std::nullopt;
        ...
        thing.platformName = platform ? QString::fromStdString(platform->name) : "Unknown";
        thing.secondsPlayed = static_cast<int>(it.value() / 1000);
        thing.percentPlayed = dayTotalMs > 0 ? static_cast<float>(it.value()) / static_cast<float>(dayTotalMs) : 0.0f;
