...
namespace firelight::gui {
    ...
    public:
        ...
        Q_PROPERTY(int maxResultsPerCategory
            READ maxResultsPerCategory
            WRITE setMaxResultsPerCategory
            NOTIFY maxResultsPerCategoryChanged)
        ...
        static int calculateFuzzyScore(const QString &text, const QString &pattern) {
            ...
            while (tIdx < t.length() && pIdx < p.length()) {
                if (t.at(tIdx) == p.at(pIdx)) {
                    ...
                    if (consecutiveMatches > 0) score += 5; // Consecutive streak bonus
