...
namespace firelight::gui {
    ...
    QImage GameImageProvider::requestImage(const QString &id, QSize *size, const QSize &requestedSize) {
        ...
        if (requestedSize.height() > 0) {
            return m_images[id].scaledToHeight(requestedSize.height(), Qt::SmoothTransformation);
        }

        if (requestedSize.width() > 0) {
            return m_images[id].scaledToWidth(requestedSize.width(), Qt::SmoothTransformation);
        }
    ...
    QString GameImageProvider::setImage(const QImage &image) {
        if (m_cacheIdsToProviderIds.contains(image.cacheKey())) {
            return "image://gameimages/" + m_cacheIdsToProviderIds[image.cacheKey()];
        }
