...
class SaveFilesItem : public QObject, public ServiceAccessor {
  ...
  Q_PROPERTY(int activeSaveSlotNumber READ getActiveSaveSlotNumber WRITE setActiveSaveSlotNumber NOTIFY
                 activeSaveSlotNumberChanged)
  Q_PROPERTY(QAbstractListModel *saveFiles READ getSaveFiles NOTIFY saveFilesChanged)

public:
  explicit SaveFilesItem(QObject *parent = nullptr) : QObject(parent) { m_saveFilesModel = new SavefileListModel(); }
