...
KeyboardMappingItem::KeyboardMappingItem(QObject *parent) { refreshMapping(); }

QVariantMap KeyboardMappingItem::getInputMappings() const {
  return m_inputMappings;
}

int KeyboardMappingItem::getProfileId() const { return m_profileId; }

int KeyboardMappingItem::getPlatformId() const { return m_platformId; }
...
void KeyboardMappingItem::addMapping(int input, const int mappedInput) {
  m_inputMapping->addKeyboardMapping(static_cast<GamepadInput>(input),
                                     mappedInput);
  m_inputMapping->sync();
...
void KeyboardMappingItem::removeAllMappings() {
  for (auto key : m_inputMappings.keys()) {
    m_inputMapping->removeMapping(static_cast<GamepadInput>(key.toInt()));
...
void KeyboardMappingItem::removeMapping(int input) {
  m_inputMapping->removeMapping(static_cast<GamepadInput>(input));
  m_inputMapping->sync();
...
QString KeyboardMappingItem::getDefaultKeyLabel(int input) const {
  ...
  case libretro::IRetroPad::SouthFace:
    return "Z";
  case libretro::IRetroPad::LeftStickUp:
    return "Up";
  case libretro::IRetroPad::LeftStickDown:
    return "Down";
  case libretro::IRetroPad::LeftStickLeft:
    return "Left";
  case libretro::IRetroPad::LeftStickRight:
    return "Right";
...
void KeyboardMappingItem::refreshMapping() {
  ...
  // m_inputMapping = std::static_pointer_cast<KeyboardMapping>(
  //     getControllerRepository()->getInputMapping(m_profileId, m_platformId));
  // for (const auto &[input, mappedInput] :
  //      m_inputMapping->getKeyboardMappings()) {
  //   m_inputMappings.insert(QString::number(input), mappedInput);
  // }
  emit inputMappingsChanged();
}
