...
void SdlController::setControllerProfile(
    const std::shared_ptr<input::ControllerProfile> &profile) {
  m_profile = profile;
}

void SdlController::setActiveMapping(
    const std::shared_ptr<input::InputMapping> &mapping) {
  m_activeMapping = mapping;
}
std::vector<Shortcut> SdlController::getToggledShortcuts(int button) {
  if (isButtonPressed(-1, LeftTrigger) &&
      button == SDL_CONTROLLER_BUTTON_BACK) {
    return {OpenRewindMenu};
  }
...
bool SdlController::isButtonPressed(const int platformId,
...
int32_t SdlController::getDeviceIndex() const {
  return m_SDLJoystickDeviceIndex;
}
...
int SdlController::getProfileId() const { return m_profile->getId(); }
} // namespace firelight::input
