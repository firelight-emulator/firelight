...
namespace firelight::library {
    RomFile::RomFile(const QString &path) : m_filePath(path) {
        ...
        m_fileMd5 = QCryptographicHash::hash(bytes, QCryptographicHash::Algorithm::Md5).toHex();

        const auto crc = crc32(0L, nullptr, 0);
        m_fileCrc32 = QString::number(crc32(crc, reinterpret_cast<const Bytef *>(bytes.data()), bytes.size()));
    ...
    RomFile::RomFile(const QString &filename, const char *data, const size_t size,
                     const QString &archivePathName) : m_filePath(filename) {
        if (!archivePathName.isEmpty()) {
            m_archivePathName = archivePathName;
            m_inArchive = true;
        }

        m_suffix = filename.right(filename.length() - (filename.lastIndexOf('.') + 1)).toLower();
        ...
        const auto crc = crc32(0L, nullptr, 0);
        m_fileCrc32 = QString::number(crc32(crc, reinterpret_cast<const Bytef *>(data), size));
    ...
    bool RomFile::isValid() const {
        return m_isValid;
    }

    QByteArray RomFile::getContentBytes() {
        return m_contentBytes;
    }

    QString RomFile::getContentHash() {
        return m_contentHash;
    }

    QString RomFile::getFileMd5() {
        return m_fileMd5;
    }

    QString RomFile::getFileCrc32() {
        return m_fileCrc32;
    }

    QString RomFile::getFilePath() {
        return m_filePath;
    }

    size_t RomFile::getFileSizeBytes() const {
        return m_fileSizeBytes;
    }

    bool RomFile::inArchive() const {
        return m_inArchive;
    }

    QString RomFile::getArchivePathName() {
        return m_archivePathName;
    }

    int RomFile::getPlatformId() const {
        return m_platformId;
    }
        ...
        switch (m_platformId) {
            ...
            // Disregard the header for hashing but keep it for the content.
                if (fileBytes.startsWith("NES\x1A")) {
                    m_contentHash = QCryptographicHash::hash(fileBytes.mid(16), QCryptographicHash::Algorithm::Md5).
                            toHex();
                } else {
                    m_contentHash = QCryptographicHash::hash(fileBytes, QCryptographicHash::Algorithm::Md5).toHex();
                }
                return;
            case PlatformMetadata::PLATFORM_ID_SNES:
                ...
                if (fileBytes.size() % 1024 == 512) {
                    m_contentHash = QCryptographicHash::hash(fileBytes.mid(512), QCryptographicHash::Algorithm::Md5).
                            toHex();
                } else {
                    m_contentHash = QCryptographicHash::hash(fileBytes, QCryptographicHash::Algorithm::Md5).toHex();
                }
            ...
            case PlatformMetadata::PLATFORM_ID_N64: {
                ...
                if (fileBytes.startsWith(V64_SIGNATURE)) {
                    ...
                    m_contentHash = QCryptographicHash::hash(other, QCryptographicHash::Algorithm::Md5).toHex();
            ...
            default:
                ...
                char hash[33];
                rc_hash_generate_from_buffer(hash, rcConsoleId, reinterpret_cast<const uint8_t *>(fileBytes.data()),
                                             fileBytes.size());
