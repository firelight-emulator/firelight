...
namespace firelight::library {
  SqliteUserLibrary::SqliteUserLibrary(QString path, QString mainGameDirectory)
    ...
    QSqlQuery addFavoriteColumn(getDatabase());
    addFavoriteColumn.prepare(
      "ALTER TABLE entriesv1 ADD COLUMN favorite INTEGER NOT NULL DEFAULT 0;");
    addFavoriteColumn.exec();
    ...
    connect(
      ...
      [&](const int id, const QString &romPath, const int platformId,
        ...
        if (auto entry = getEntryWithContentHash(contentHash)) {
          ...
          auto newEntry =
              Entry{
                .displayName = romPath.split("/").last(),
