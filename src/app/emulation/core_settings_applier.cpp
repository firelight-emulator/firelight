...
CoreSettingsApplier::CoreSettingsApplier(EmulatorInstance &instance,
...
void CoreSettingsApplier::refresh() {
  ...
  apply("aspect-ratio",
        [&] { m_instance.setAspectRatioMode(value("aspect-ratio")); });
  apply("integer-scale",
        [&] { m_instance.setIntegerScale(intValue("integer-scale")); });
  apply("sync-method", [&] { m_instance.setSyncMethod(value("sync-method")); });
  apply("target-framerate",
        [&] { m_instance.setTargetFramerate(intValue("target-framerate")); });
