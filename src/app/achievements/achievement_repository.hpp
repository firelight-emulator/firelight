 ...
 *
 * Defines the contract for persistent storage and retrieval of achievement-related data
 * including achievement sets, individual achievements, user progress, and content hash mappings.
 * This interface supports both local offline storage and synchronization with remote services.
 */
...
public:
   ...
   *
   * Stores an achievement set with its metadata. Uses upsert semantics - if an achievement
   * set with the same ID already exists, it will be updated with the new data.
   *
   ...
   *
   * Modifies the metadata of an existing achievement set. The achievement set must already
   * exist in the repository.
   *
   * @param achievementSet The achievement set with updated data
   * @return true if the update succeeded, false if the set doesn't exist or update failed
   */
   ...
   *
   * Fetches a complete achievement set including all associated achievements ordered by
   * their display order. Only achievements with active flags are included.
   *
   ...
   * This enables automatic achievement loading when a game is launched based on its
   * content fingerprint. Only achievements with active flags (flags == 3) are included.
   *
   ...
   *
   * Stores an achievement with all its metadata including name, description, points,
   * icon URL, type, display order, and flags. Uses upsert semantics.
   *
   ...
   *
   * Tracks incremental progress toward achievement completion for a specific user.
   * Uses upsert semantics based on the combination of username and achievement ID.
   *
   ...
   * This enables automatic achievement loading when games are launched. Uses upsert
   * semantics - if the hash already exists, it will be remapped to the new set ID.
   *
   ...
   */
  virtual bool setGameHash(int setId, const std::string &contentHash) = 0;
   ...
   *
   * Gets the unlock status and metadata for a user's achievement, including both
   * normal and hardcore mode completion status, timestamps, and sync status.
   *
   ...
   *
   * Stores the complete patch response data for offline use, enabling achievement
   * functionality when not connected to the RetroAchievements service.
   *
