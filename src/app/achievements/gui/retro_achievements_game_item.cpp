...
QString RetroAchievementsGameItem::getContentHash() const { return m_contentHash; }
...
void RetroAchievementsGameItem::setContentHash(const QString &contentHash) {
  ...
  const auto entry = getLibraryService()->getEntryWithContentHash(contentHash.toStdString());
  if (!entry) {
  ...
  auto game = getAchievementService()->getGameForHash(contentHash.toStdString());
  if (!game) {
    m_achievementSets.clear();
    return;
  }
...
QList<AchievementSetItem *> RetroAchievementsGameItem::getAchievementSets() const { return m_achievementSets; }
} // namespace firelight::achievements
