...
int CoreInputRouter::getPortInputClass(const unsigned port) const {
  ...
  return it != m_portInputClass.end() ? it->second : static_cast<int>(firelight::input::GamepadInputClass::Joypad);
}
...
void CoreInputRouter::pollInput() {
  ...
  if (m_pointerInputProvider && m_retropadProvider) {
    ...
    for (const auto &[port, deviceClass] : m_portInputClass) {
      ...
      auto pad = m_retropadProvider->getRetropadForPlayerIndex(static_cast<int>(port));
      if (!pad) {
      ...
      m_pointerInputProvider->nudgeCursor(cursorGlideDelta(stickX, m_analogPointerSpeed),
  ...
  for (int port = 0; port < MAX_INPUT_PORTS; ++port) {
    const auto pad = m_retropadProvider ? m_retropadProvider->getRetropadForPlayerIndex(port) : nullptr;
    if (pad) {
      m_portFrames[port] = firelight::input::captureJoypadFrame(*pad, m_platformId, 1);
...
int16_t CoreInputRouter::readInputState(const unsigned port, const unsigned device, const unsigned index,
  ...
  if (deviceClass == RETRO_DEVICE_MOUSE || deviceClass == RETRO_DEVICE_LIGHTGUN) {
    namespace fi = firelight::input;
    ...
    const bool gamepadAllowed =
        (deviceClass == RETRO_DEVICE_MOUSE && selectedClass == static_cast<int>(fi::GamepadInputClass::Mouse)) ||
        (deviceClass == RETRO_DEVICE_LIGHTGUN && selectedClass == static_cast<int>(fi::GamepadInputClass::Lightgun));
    ...
    const auto pad = provider ? provider->getRetropadForPlayerIndex(port) : nullptr;
    const int mapClass = static_cast<int>(deviceClass == RETRO_DEVICE_MOUSE ? fi::GamepadInputClass::Mouse
                                                                            : fi::GamepadInputClass::Lightgun);
    const auto mapped = [&](const fi::GamepadInput vi) -> bool {
      return gamepadAllowed && pad && pad->isVirtualInputActive(m_platformId, mapClass, static_cast<int>(vi));
    };
    ...
    if (deviceClass == RETRO_DEVICE_MOUSE) {
      ...
      case RETRO_DEVICE_ID_MOUSE_MIDDLE:
        return mouseBtn(RETRO_DEVICE_ID_MOUSE_MIDDLE) || mapped(fi::MouseMiddle);
      default:
        return 0;
      }
    }
    ...
    case RETRO_DEVICE_ID_LIGHTGUN_TRIGGER:
      return (mouseAllowed && pointer && pointer->isPressed()) || mapped(fi::LightgunTrigger);
