...
Core::Core(firelight::libretro::CoreRunConfig config)
    : m_platformId(config.platformId), m_configurationProvider(std::move(config.configProvider)),
      systemDirectory(std::move(config.systemDirectory)), m_saveDirectory(std::move(config.saveDirectory)) {
...
bool Core::loadGame(Game *game) {
  ...
  info.path = strdup(std::filesystem::path(game->getPath()).string().c_str());
  // info.path = R"()";
...
bool Core::deserializeState(const std::vector<uint8_t> &data) const {
  ...
  if (data.size() != size) {
    spdlog::error("[Core] Refusing to load state: size mismatch (data: {}, expected: {})", data.size(), size);
    return false;
  }
...
void Core::setSystemDirectory(const string &frontendSystemDirectory) { systemDirectory = frontendSystemDirectory; }

void Core::setSaveDirectory(const string &frontendSaveDirectory) { m_saveDirectory = frontendSaveDirectory; }

void Core::recordPotentialAPIViolation(const std::string &msg) { printf("Potential API violation: %s\n", msg.c_str()); }
...
std::vector<char> Core::getMemoryData(const MemoryType memType) const {
  ...
  const auto ptr = static_cast<char *>(m_dll->getMemoryData(static_cast<unsigned>(memType)));

  return std::vector(ptr, ptr + size);
}

void Core::writeMemoryData(const MemoryType memType, const std::vector<char> &data) {
  const auto size = m_dll->getMemorySize(static_cast<unsigned>(memType));
...
void *Core::getMemoryData(const unsigned id) const { return m_dll->getMemoryData(id); }

size_t Core::getMemorySize(const unsigned id) const { return m_dll->getMemorySize(id); }
...
bool Core::setDiskIndex(const unsigned index) {
  ...
  const auto setEject = m_hasDiskControlExt ? m_diskControlExt.set_eject_state
                                            : (m_hasDiskControl ? m_diskControl.set_eject_state : nullptr);
  const auto setIndex = m_hasDiskControlExt ? m_diskControlExt.set_image_index
                                            : (m_hasDiskControl ? m_diskControl.set_image_index : nullptr);
...
std::vector<std::vector<Core::ControllerDeviceOption>> Core::getControllerDevices() const {
  return m_controllerDevices;
}

void Core::setControllerPortDevice(const unsigned port, const unsigned device) {
...
void Core::setAnalogPointerSpeed(const double stepPerFrame) { m_input.setAnalogPointerSpeed(stepPerFrame); }

void Core::setMouseControlsPointerDevices(const bool enabled) { m_input.setMouseControlsPointerDevices(enabled); }

int Core::getPortInputClass(const unsigned port) const { return m_input.getPortInputClass(port); }

void Core::setCheat(const unsigned index, const bool enabled, const std::string &code) {
  m_dll->cheatSet(index, enabled, code.c_str());
}

void Core::clearCheats() { m_dll->cheatReset(); }

void Core::sendKeyboardEvent(const bool down, const unsigned key, const uint32_t character, const uint16_t modifiers) {
  if (keyboardCallback) {
...
void Core::setRetropadProvider(firelight::libretro::IRetropadProvider *provider) {
...
void Core::setAudioInputProvider(firelight::libretro::IAudioInputProvider *provider) {
