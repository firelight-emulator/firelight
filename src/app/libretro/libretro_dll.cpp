...
LibretroDll::LibretroDll(const std::string &path) {
  ...
  if (!m_lib->load()) {
    throw std::runtime_error("Failed to load libretro core '" + path + "': " + m_lib->errorString().toStdString());
  }
  ...
  m_getSystemInfo = reinterpret_cast<void (*)(retro_system_info *)>(req("retro_get_system_info"));
  m_getSystemAVInfo = reinterpret_cast<void (*)(retro_system_av_info *)>(req("retro_get_system_av_info"));
  m_setControllerPortDevice = reinterpret_cast<void (*)(unsigned, unsigned)>(req("retro_set_controller_port_device"));
  m_reset = reinterpret_cast<void (*)()>(req("retro_reset"));
  m_run = reinterpret_cast<void (*)()>(req("retro_run"));
  m_serializeSize = reinterpret_cast<size_t (*)()>(req("retro_serialize_size"));
  m_serialize = reinterpret_cast<bool (*)(void *, size_t)>(req("retro_serialize"));
  m_unserialize = reinterpret_cast<bool (*)(const void *, size_t)>(req("retro_unserialize"));
  m_cheatReset = reinterpret_cast<void (*)()>(req("retro_cheat_reset"));
  m_cheatSet = reinterpret_cast<void (*)(unsigned, bool, const char *)>(req("retro_cheat_set"));
  m_loadGame = reinterpret_cast<bool (*)(const retro_game_info *)>(req("retro_load_game"));
  m_loadGameSpecial =
      reinterpret_cast<bool (*)(unsigned, const retro_game_info *, size_t)>(req("retro_load_game_special"));
  m_unloadGame = reinterpret_cast<void (*)()>(req("retro_unload_game"));
  m_getRegion = reinterpret_cast<unsigned (*)()>(req("retro_get_region"));
  m_getMemoryData = reinterpret_cast<void *(*)(unsigned)>(req("retro_get_memory_data"));
  m_getMemorySize = reinterpret_cast<size_t (*)(unsigned)>(req("retro_get_memory_size"));

  m_setEnvironment = reinterpret_cast<void (*)(retro_environment_t)>(req("retro_set_environment"));
  m_setVideoRefresh = reinterpret_cast<void (*)(retro_video_refresh_t)>(req("retro_set_video_refresh"));
  m_setAudioSample = reinterpret_cast<void (*)(retro_audio_sample_t)>(req("retro_set_audio_sample"));
  m_setAudioSampleBatch = reinterpret_cast<void (*)(retro_audio_sample_batch_t)>(req("retro_set_audio_sample_batch"));
  m_setInputPoll = reinterpret_cast<void (*)(retro_input_poll_t)>(req("retro_set_input_poll"));
  m_setInputState = reinterpret_cast<void (*)(retro_input_state_t)>(req("retro_set_input_state"));
  ...
  if (!missing.empty()) {
    ...
    m_lib->unload();
    throw std::runtime_error("libretro core '" + path + "' is missing required symbol(s): " + joined);
  }
}
...
void LibretroDll::getSystemAVInfo(retro_system_av_info *info) { m_getSystemAVInfo(info); }
...
bool LibretroDll::unserialize(const void *data, size_t size) { return m_unserialize(data, size); }
...
void LibretroDll::setVideoRefresh(retro_video_refresh_t cb) { m_setVideoRefresh(cb); }

void LibretroDll::setAudioSample(retro_audio_sample_t cb) { m_setAudioSample(cb); }

void LibretroDll::setAudioSampleBatch(retro_audio_sample_batch_t cb) { m_setAudioSampleBatch(cb); }
