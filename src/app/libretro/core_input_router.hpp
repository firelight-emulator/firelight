...
public:
  ...
  void setPointerInputProvider(IPointerInputProvider *provider) {
    m_pointerInputProvider = provider;
  }
  [[nodiscard]] IRetropadProvider *getRetropadProvider() const {
    return m_retropadProvider;
  }
  [[nodiscard]] IPointerInputProvider *getPointerInputProvider() const {
  ...
  void setMouseControlsPointerDevices(const bool enabled) {
