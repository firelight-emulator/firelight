...
public:
  ...
  void setVideoReceiver(firelight::libretro::IVideoDataReceiver *receiver) override;

  void setRetropadProvider(firelight::libretro::IRetropadProvider *provider) override;

  void setPointerInputProvider(firelight::libretro::IPointerInputProvider *provider) override;

  void setAudioInputProvider(firelight::libretro::IAudioInputProvider *provider) override;
  ...
  [[nodiscard]] std::vector<char> getMemoryData(MemoryType memType) const override;

  void writeMemoryData(MemoryType memType, const std::vector<char> &data) override;
  ...
  std::vector<std::vector<ControllerDeviceOption>> getControllerDevices() const override;
  ...
  [[nodiscard]] bool mouseControlsPointerDevices() const { return m_input.mouseControlsPointerDevices(); }

  void setCheat(unsigned index, bool enabled, const std::string &code) override;

  void clearCheats() override;

  void sendKeyboardEvent(bool down, unsigned key, uint32_t character, uint16_t modifiers) override;

  [[nodiscard]] bool wantsKeyboard() const override { return keyboardCallback != nullptr; }
