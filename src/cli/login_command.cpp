...
int runLogin(int argc, char **argv, const CliOptions &opts) {
  ...
  QSettings::setPath(QSettings::Format::IniFormat, QSettings::Scope::UserScope, dirs.appDataPath);
  ...
  QObject::connect(&raClient, &firelight::achievements::RAClient::loginSucceeded, &loop, [&] {
    result = 0;
    done = true;
    loop.quit();
  });
  ...
  QObject::connect(&raClient, &firelight::achievements::RAClient::loginFailedWithInvalidCredentials, &loop,
                   [&] { fail("invalid credentials"); });
  QObject::connect(&raClient, &firelight::achievements::RAClient::loginFailedWithExpiredToken, &loop,
                   [&] { fail("expired token"); });
  QObject::connect(&raClient, &firelight::achievements::RAClient::loginFailedWithAccessDenied, &loop,
                   [&] { fail("access denied"); });
  QObject::connect(&raClient, &firelight::achievements::RAClient::loginFailedWithInternalError, &loop,
                   [&] { fail("internal error"); });
