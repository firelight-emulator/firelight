...
#define FL_VERSION "0.0.0"
  ...
  if (!QFileInfo::exists(defaultAppDataPathString) && QDir().mkpath(defaultAppDataPathString)) {
    spdlog::info("[Startup] App data directory does not exist; creating: {}", defaultAppDataPathString.toStdString());
  }
  ...
  // future instances
  // ============================================================================================
  const auto singleInstanceName = firelight::cli::singleInstanceServerName(defaultAppDataPathString);
  if (cliOptions.singleInstance && forwardLaunchToRunningInstance(singleInstanceName, cliOptions)) {
    return 0;
  }
  ...
  firelight::media::MediaService mediaService(capturesPath, gameCaptureRepository);
  ...
  firelight::saves::SqliteSaveDatabase saveDatabase(userdataDbPath.toStdString());
  ...
  const auto resolvedSaveDir = savesSettings.value("Saves/SaveDirectory", savesPath).toString().toStdString();
  ...
  firelight::achievements::RetroAchievementsOfflineClient offlineRaClient(achievementService);
  ...
  const auto shortcutsPath = (QCoreApplication::applicationDirPath() + "/system/shortcuts.json").toStdString();
  ...
  firelight::ServiceAccessor::setControllerProfileRepository(&controllerRepository);
  ...
  const auto contentDirAddedConn =
      EventDispatcher::instance().subscribe<firelight::library::ContentDirectoryAddedEvent>(
          [&](const firelight::library::ContentDirectoryAddedEvent &e) {
            libScanner2.watchPath(QString::fromStdString(e.path));
            libScanner2.scanAll();
          });
  const auto contentDirUpdatedConn =
      EventDispatcher::instance().subscribe<firelight::library::ContentDirectoryUpdatedEvent>(
  ...
  const auto contentDirRemovedConn =
      EventDispatcher::instance().subscribe<firelight::library::ContentDirectoryRemovedEvent>(
  ...
  const auto scanSuspendConn = EventDispatcher::instance().subscribe<firelight::emulation::EmulationStartedEvent>(
      [&](const firelight::emulation::EmulationStartedEvent &) { libScanner2.setScanningSuspended(true); });
  const auto scanResumeConn = EventDispatcher::instance().subscribe<firelight::emulation::EmulationStoppedEvent>(
      [&](const firelight::emulation::EmulationStoppedEvent &) { libScanner2.setScanningSuspended(false); });
  ...
  const int startupLaunchEntryId =
      firelight::cli::resolveRomEntryId(cliOptions.romPath, userLibraryService, platformService);
  ...
  if (cliOptions.singleInstance) {
    singleInstanceServer =
        std::make_unique<firelight::cli::SingleInstanceServer>(singleInstanceName, userLibraryService, platformService);
  ...
  qmlRegisterType<firelight::saves::SaveFilesItem>("Firelight", 1, 0, "SaveDataInformation");

  qmlRegisterType<firelight::input::InputMappingsModel>("Firelight", 1, 0, "InputMappingsModel");
  qmlRegisterType<firelight::input::AnalogSettingsModel>("Firelight", 1, 0, "AnalogSettingsModel");
  qmlRegisterType<firelight::input::ProfileListModel>("Firelight", 1, 0, "ProfileListModel");
  qmlRegisterType<firelight::input::BindingListModel>("Firelight", 1, 0, "BindingListModel");
  qmlRegisterType<firelight::input::PlatformInputPreferences>("Firelight", 1, 0, "PlatformInputPreferences");
  qmlRegisterType<firelight::gui::ControllerListModel>("Firelight", 1, 0, "GamepadListModel");
  qmlRegisterType<firelight::settings::SettingsModel>("Firelight", 1, 0, "SettingsModel");
  qmlRegisterType<firelight::settings::CoreOptionsModel>("Firelight", 1, 0, "CoreOptionsModel");
  qmlRegisterType<firelight::settings::SettingsSearchModel>("Firelight", 1, 0, "SettingsSearchModel");
  qmlRegisterType<firelight::settings::SettingBinding>("Firelight", 1, 0, "SettingBinding");
  ...
  QObject::connect(QNetworkInformation::instance(), &QNetworkInformation::reachabilityChanged,
                   [&raClient, &achievementService](QNetworkInformation::Reachability reachability) {
  ...
  firelight::emulation::ShortcutDispatcher shortcutDispatcher(
      shortcutActions, [&settingsService](const int playerIndex) {
        if (settingsService.getGlobalValue("only-player-one-hotkeys").value_or("false") != "true") {
          return true;
        }
  ...
  shortcutActions.setIntents({
      ...
      .notify = [&](const std::string &message) { emit shortcutDispatcher.notified(QString::fromStdString(message)); },
  });
  ...
  // These affect only this run and are never written to the settings DB
  {
    ...
    if (!cliOptions.settingsFile.empty()) {
      try {
        const auto fileOverrides = firelight::cli::loadOverrideFile(cliOptions.settingsFile);
        overrides.insert(overrides.end(), fileOverrides.begin(), fileOverrides.end());
      } catch (const std::exception &e) {
        spdlog::error("Ignoring --settings-file {}: {}", cliOptions.settingsFile, e.what());
      }
    }
    ...
    if (!cliOptions.core.empty()) {
      firelight::CoreRegistry::instance().setSessionCoreOverride(cliOptions.core);
    }
    ...
    if (!cliOptions.controller.empty()) {
      const auto type = firelight::cli::parseControllerType(cliOptions.controller);
      if (!type.has_value()) {
        spdlog::warn("--controller '{}' is not a known type (valid: {})", cliOptions.controller,
                     firelight::cli::controllerTypeNames());
      } else if (!haveRom) {
        spdlog::warn("--controller ignored: no ROM was given to launch");
      } else if (const auto entry = userLibraryService.getEntry(startupLaunchEntryId)) {
        inputService.setSessionPreferredControllerType(entry->platformId, static_cast<int>(*type));
      }
    }

    if (!cliOptions.raUsername.empty() && cliOptions.raPassword.empty() && cliOptions.raToken.empty()) {
  ...
  QObject::connect(&libraryFolderListModel, &firelight::gui::LibraryFolderListModel::folderDeleted, &entryListModel,
                   &firelight::library::EntryListModel::reset, Qt::QueuedConnection);
  ...
  engine.rootContext()->setContextProperty("SettingsCatalog", new firelight::gui::QtSettingsCatalogProxy());
  ...
  engine.rootContext()->setContextProperty("ActivityBucketsModel", activityBucketsModel);

  const auto searchResultsModel = new firelight::gui::SearchResultsListModel(userLibraryService, platformService);
  ...
  engine.rootContext()->setContextProperty("window_resize_handler", resizeHandler);
  auto inputMethodDetectionHandler = new firelight::gui::InputMethodDetectionHandler();
  engine.rootContext()->setContextProperty("InputMethodManager", inputMethodDetectionHandler);
  ...
  startupData.fullscreenOverride = cliOptions.fullscreen ? 1 : (cliOptions.windowed ? 0 : -1);
  startupData.exitOnClose = cliOptions.exitOnClose;
  startupData.raPendingLogin =
      !cliOptions.raUsername.empty() && (!cliOptions.raPassword.empty() || !cliOptions.raToken.empty());
  ...
  inputService.setKeyboard(std::shared_ptr<firelight::input::IGamepad>(keyboardHandler));
  ...
  QObject::connect(&discordCallbackTimer, &QTimer::timeout, [&] { discordManager.runCallbacks(); });
