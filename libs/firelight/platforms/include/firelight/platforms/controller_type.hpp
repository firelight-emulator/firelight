...
inline void from_json(const nlohmann::json &j, ControllerType &c) {
  ...
  c.deviceClass = static_cast<input::GamepadInputClass>(
      j.value("device_class", static_cast<int>(input::GamepadInputClass::Joypad)));
  j.at("inputs").get_to(c.inputs);
}

inline void to_json(nlohmann::json &j, const ControllerType &c) {
  j = nlohmann::json{
      {"id", c.id}, {"name", c.name}, {"device_class", static_cast<int>(c.deviceClass)}, {"inputs", c.inputs}};
}
