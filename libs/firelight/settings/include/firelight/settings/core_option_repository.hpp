...
public:
  ...
  virtual void upsertCoreOptions(const std::string &coreName, const std::vector<CoreOption> &options) = 0;

  // The cached options for a core in declaration order, or empty if none
  [[nodiscard]] virtual std::vector<CoreOption> getCoreOptions(const std::string &coreName) = 0;
};
