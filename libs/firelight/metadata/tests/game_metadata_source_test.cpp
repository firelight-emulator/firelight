...
std::string makeFixtureDb() {
  ...
  const auto path =
      (std::filesystem::temp_directory_path() / ("fl_meta_fixture_" + std::to_string(counter.fetch_add(1)) + ".db"))
          .string();
...
TEST(GameMetadataSourceTest, MissingDatabaseIsTolerated) {
  SqliteGameMetadataSource source((std::filesystem::temp_directory_path() / "fl_meta_absent_zzz.db").string());
