...
namespace firelight::metadata {
  ...
  SqliteMediaAssetRepository::listForContent(const std::string &contentHash) {
  ...
  std::vector<MediaAsset> SqliteMediaAssetRepository::listForContentAndType(
    const std::string &contentHash, MediaType type) {
    std::lock_guard lock(m_mutex);
    std::vector<MediaAsset> assets;
    try {
      SQLite::Statement query(
        *m_db, "SELECT * FROM media_assets WHERE content_hash = :contentHash AND "
      ...
      spdlog::error("Failed to list media for {} type {}: {}", contentHash,
                    static_cast<int>(type), e.what());
    }
    return assets;
  }

  std::optional<MediaAsset>
  SqliteMediaAssetRepository::selectedFor(const std::string &contentHash,
                                          MediaType type) {
    std::lock_guard lock(m_mutex);
    try {
      SQLite::Statement query(
      ...
      spdlog::error("Failed to get selected media for {}: {}", contentHash,
                    e.what());
    }
    return std::nullopt;
  }
    ...
    try {
      {
        SQLite::Statement insert(
          *m_db,
          "INSERT OR IGNORE INTO media_assets(content_hash, media_type, source, "
          "remote_url, thumb_url, local_path, width, height, external_id, "
          "selected, created_at) VALUES(:contentHash, :mediaType, :source, :remoteUrl, :thumbUrl, :localPath, :width, :height, "
        ...
        insert.bind(":createdAt",
                    static_cast<int64_t>(asset.createdAt
                                           ? asset.createdAt
                                           : nowMs()));
        insert.exec();
      }
      ...
      if (asset.id <= 0) {
        ...
        SQLite::Statement update(
          *m_db, "UPDATE media_assets SET thumb_url = :thumbUrl, local_path = :localPath, "
      ...
      spdlog::error("Failed to add media asset for {}: {}", asset.contentHash,
                    e.what());
      return false;
    }
  }
    ...
    try {
      ...
      int type; {
        ...
        SQLite::Statement set(
          *m_db, "UPDATE media_assets SET selected = 1 WHERE id = :id");
        set.bind(":id", assetId);
        set.exec();
      }
