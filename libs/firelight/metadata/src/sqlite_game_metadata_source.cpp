...
namespace firelight::metadata {
  SqliteGameMetadataSource::SqliteGameMetadataSource(std::string databaseFile)
    : m_databaseFile(std::move(databaseFile)) {
    try {
      m_db = std::make_unique<SQLite::Database>(m_databaseFile,
                                                SQLite::OPEN_READONLY);
    } catch (const std::exception &e) {
      // It's fine if we don't find it, we just won't have metadata available
      spdlog::warn("Game metadata database unavailable ({}): {}", m_databaseFile,
                   e.what());
      m_db.reset();
    }
  }
  ...
  std::optional<GameMetadata>
  SqliteGameMetadataSource::lookup(const std::string &contentHash) {
    if (!m_db) {
      return std::nullopt;
    }
    try {
      SQLite::Statement query(
        *m_db,
        "SELECT g.id, g.name, g.description, g.developer, g.publisher, g.genre, "
      ...
      SQLite::Statement mediaQuery(
        *m_db, "SELECT media_type, url FROM game_media WHERE game_id = :gid");
      mediaQuery.bind(":gid", gameId);

      while (mediaQuery.executeStep()) {
        metadata.media.push_back(MediaDefault{
          .type = static_cast<MediaType>(mediaQuery.getColumn(0).getInt()),
