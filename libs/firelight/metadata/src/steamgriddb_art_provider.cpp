...
std::vector<SteamGridDbArtProvider::GameMatch> SteamGridDbArtProvider::resolveGames(const std::string &gameName,
...
void SteamGridDbArtProvider::collectArtForGame(const GameMatch &game, const MediaType type, const int remaining,
  ...
  const auto response = apiGet("/" + endpoint + "/game/" + std::to_string(game.id));
...
std::vector<ArtCandidate> SteamGridDbArtProvider::search(const std::string &gameName, int platformId,
