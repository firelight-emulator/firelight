...
static void eventHandler(const rc_client_event_t *event, rc_client_t *client) {
  ...
  case RC_CLIENT_EVENT_ACHIEVEMENT_TRIGGERED: {
    ...
    auto success =
        rc_client_achievement_get_image_url(event->achievement, RC_CLIENT_ACHIEVEMENT_STATE_UNLOCKED, urlBuffer, 256);
    emit raClient->achievementUnlocked(QString(urlBuffer), QString(event->achievement->title),
  ...
  case RC_CLIENT_EVENT_ACHIEVEMENT_CHALLENGE_INDICATOR_SHOW: {
    ...
    emit raClient->showChallengeIndicator(event->achievement->id, QString(urlBuffer),
                                          QString(event->achievement->title), QString(event->achievement->description));
    break;
  }
  ...
  case RC_CLIENT_EVENT_ACHIEVEMENT_PROGRESS_INDICATOR_UPDATE:
    if (event->achievement->measured_progress[0] != '\0') {
      ...
      if (progress.find('/') != string::npos) {
        ...
        if (std::ranges::all_of(current, isdigit) && std::ranges::all_of(desired, isdigit)) {
          char urlBuffer[256];

          auto success =
              rc_client_achievement_get_image_url(event->achievement, event->achievement->unlocked, urlBuffer, 256);

          emit raClient->achievementProgressUpdated(QString(urlBuffer), event->achievement->id,
                                                    event->achievement->title, event->achievement->description,
                                                    std::stoi(current), std::stoi(desired));

          raClient->m_service.create(AchievementProgress{.username = raClient->getCurrentUser()->username,
    ...
    emit raClient->achievementProgressPercentUpdated(event->achievement->id, event->achievement->measured_percent);
  ...
  case RC_CLIENT_EVENT_GAME_COMPLETED: {
    ...
    char urlBuffer[256];
    auto imageUrlResult = rc_client_game_get_image_url(gameInfo, urlBuffer, 256);

    if (raClient->hardcoreModeActive()) {
      emit raClient->gameMastered(urlBuffer, QString(gameInfo->title),
                                  "You got all the achievements in hardcore mode!");
    } else {
      emit raClient->gameBeaten(urlBuffer, QString(gameInfo->title), "You got all the achievements in the game!");
    }
    break;
  }
...
static uint32_t readMemoryCallback(uint32_t address, uint8_t *buffer, uint32_t num_bytes, rc_client_t *client) {
  ...
  if (raClient && theCore) {
    if (!raClient->m_memorySeemsGood) {
      ...
      const auto valid = rc_libretro_memory_init(
          ...
          [](unsigned id, rc_libretro_core_memory_info_t *info) {
            info->data = static_cast<unsigned char *>(theCore->getMemoryData(id));
    ...
    if (raClient->m_memorySeemsGood) {
      return rc_libretro_memory_read(&raClient->m_memoryRegions, address, buffer, num_bytes);
    }
    return 0;
  }
...
static void httpCallback(const rc_api_request_t *request, rc_client_server_callback_t callback, void *callback_data,
                         rc_client_t *client) {
  ...
  QThreadPool::globalInstance()->start([=] {
    if (!postData.empty()) {
      ...
      const auto response = raClient->m_httpClient->handleRequest(url, postData, contentType);
...
RAClient::RAClient(RetroAchievementsOfflineClient &offlineClient, AchievementService &service)
  ...
  connect(&m_idleTimer, &QTimer::timeout, this, [this] { rc_client_idle(m_client); });

  m_settings = std::make_unique<QSettings>();
  m_settings->beginGroup("RetroAchievements");

  m_progressNotificationsEnabled = m_settings->value("ProgressNotificationsEnabled", true).toBool();
  m_unlockNotificationsEnabled = m_settings->value("UnlockNotificationsEnabled", true).toBool();
  m_challengeIndicatorsEnabled = m_settings->value("ChallengeIndicatorsEnabled", true).toBool();
...
void RAClient::loadGame(int platformId, const std::string &contentMd5) {
  ...
  m_consoleId = platforms::PlatformService::rcConsoleForPlatform(platformId);

  rc_client_begin_load_game(
      m_client, contentMd5.c_str(),
      [](int result, const char *error_message, rc_client_t *client, void *userdata) {
        const auto theThing = static_cast<RAClient *>(rc_client_get_userdata(client));
        if (result == 0) {
          ...
          emit theThing->gameLoadSucceeded(buffer, QString(gameInfo->title), numEarned, numTotal);
          ...
          if (theThing->m_connected && rc_client_get_hardcore_enabled(client)) {
            ...
            theThing->m_service.startSession(theThing->m_displayName.toStdString(), gameInfo->id, true);
          } else {
            theThing->m_service.startSession(theThing->m_displayName.toStdString(), gameInfo->id, false);
          }
        } else {
...
bool RAClient::updateRichPresenceMessage() {
  if (!m_loggedIn || !m_gameLoaded) {
    return false;
  }

  if (!rc_client_has_rich_presence(m_client)) {
    return false;
  }

  char buffer[256];
  auto size = rc_client_get_rich_presence_message(m_client, buffer, 256);

  if (size == 0) {
    return false;
  }

  auto newMessage = std::string(buffer, size);
  if (newMessage != m_richPresenceMessage) {
    m_richPresenceMessage = newMessage;
    EventDispatcher::instance().publish(RichPresenceMessageChangedEvent{m_richPresenceMessage});
    return true;
  }
...
void RAClient::deserializeState(const std::vector<uint8_t> &state) {
  ...
  if (rc_client_deserialize_progress_sized(m_client, state.data(), state.size()) != RC_OK) {
...
void RAClient::setOnlineForTesting(const bool online) const { m_httpClient->setOnlineForTesting(online); }
...
void RAClient::logInUserWithPassword(const QString &username, const QString &password) {
  ...
  rc_client_begin_login_with_password(
      ...
      [](int result, const char *error_message, rc_client_t *client, void *userdata) {
        ...
        case RC_OK:
          ...
          raClient->m_service.setLoggedInUsername(raClient->m_displayName.toStdString());
          emit raClient->loginSucceeded();
          emit raClient->loginStatusChanged();
          emit raClient->pointsChanged();

          raClient->m_settings->setValue("Username", QString(userInfo->username));
          raClient->m_settings->setValue("ConnectToken", QString(userInfo->token));
          break;
...
void RAClient::logInUserWithToken(const QString &username, const QString &token) {
  m_expectToBeLoggedIn = true;
