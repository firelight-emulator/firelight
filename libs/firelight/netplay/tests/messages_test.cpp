...
TEST(MessagesTest, HelloRoundTrips) {
  const Hello hello{.proto = PROTOCOL_VERSION, .appVersion = "1.2.3", .memberId = 42};
...
TEST(MessagesTest, WelcomeCarriesFullSnapshot) {
  ...
  welcome.table.assign(0, SlotAssignment{.memberId = 1, .localPadIndex = 0, .displayName = "Host", .ready = true});
  welcome.table.assign(3, SlotAssignment{.memberId = 2, .localPadIndex = 1, .displayName = "Guest"});
  welcome.game = SessionDescriptor{.gameName = "Mario Kart",
                                   .contentHash = "abc123",
                                   .platformId = 4,
                                   .artUrl = "http://art",
                                   .strategy = SyncStrategyKind::HostStream};
  welcome.streamConfig = StreamConfig{.extradataB64 = "c3BzcHBz", .width = 240, .height = 160, .fps = 60};
...
TEST(MessagesTest, AllSimpleTypesRoundTrip) {
  EXPECT_TRUE(std::holds_alternative<Reject>(*decodeMessage(encodeMessage(Reject{.code = "x", .message = "y"}))));
  EXPECT_TRUE(std::holds_alternative<ReadyState>(*decodeMessage(encodeMessage(ReadyState{.ready = true}))));
  EXPECT_TRUE(std::holds_alternative<SessionStarting>(*decodeMessage(encodeMessage(SessionStarting{}))));
  EXPECT_TRUE(std::holds_alternative<StreamStart>(*decodeMessage(encodeMessage(StreamStart{}))));
  EXPECT_TRUE(std::holds_alternative<PauseState>(*decodeMessage(encodeMessage(PauseState{.paused = true}))));
  EXPECT_TRUE(std::holds_alternative<SessionEnded>(*decodeMessage(encodeMessage(SessionEnded{.reason = "done"}))));
  EXPECT_TRUE(std::holds_alternative<Ping>(*decodeMessage(encodeMessage(Ping{.t = 123}))));
  EXPECT_TRUE(std::holds_alternative<Pong>(*decodeMessage(encodeMessage(Pong{.t = 123}))));
  EXPECT_TRUE(std::holds_alternative<GameSelected>(
      *decodeMessage(encodeMessage(GameSelected{.game = {.gameName = "Tetris"}}))));
  EXPECT_TRUE(std::holds_alternative<StreamConfig>(*decodeMessage(encodeMessage(StreamConfig{.width = 320}))));
}
