...
public:
  explicit ShortcutMapping(std::function<void(ShortcutMapping &)> syncCallback = nullptr);

  [[nodiscard]] const std::vector<InputSource> &getBindings(const ShortcutId &id) const;
  ...
  [[nodiscard]] const std::map<ShortcutId, std::vector<InputSource>> &getAll() const;
