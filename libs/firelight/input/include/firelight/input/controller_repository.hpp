...
public:
  ...
  // --- Devices ---
  virtual std::optional<DeviceInfo> getDeviceInfo(DeviceIdentifier identifier) const = 0;

  virtual void updateDeviceInfo(DeviceIdentifier identifier, const DeviceInfo &info) = 0;
  ...
  // next launch
  virtual std::shared_ptr<GamepadProfile> createProfile(std::string name, DeviceType device = DeviceType::Gamepad) = 0;
  ...
  virtual std::shared_ptr<GamepadProfile> cloneProfile(int sourceId, std::string newName) = 0;
  ...
  virtual void setProfileAnalogSettings(int profileId, const AnalogSettings &settings) = 0;
  ...
  virtual std::shared_ptr<GamepadProfile> importProfile(const std::string &json) = 0;
  ...
  virtual std::optional<int> getPlatformPreferredType(int platformId) const = 0;

  // --- Per-game profile overrides (keyed by library content hash) ---
  virtual std::optional<int> getGameProfileOverride(const std::string &contentHash) const = 0;

  virtual void setGameProfileOverride(const std::string &contentHash, int profileId) = 0;
