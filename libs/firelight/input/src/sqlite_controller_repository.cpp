...
SqliteControllerRepository::SqliteControllerRepository(
  ...
  exec("CREATE TABLE IF NOT EXISTS profiles_v3("
  ...
  exec("CREATE TABLE IF NOT EXISTS mappings_v3("
       "id INTEGER PRIMARY KEY AUTOINCREMENT,"
  ...
  exec("CREATE TABLE IF NOT EXISTS shortcuts_v3("
       "id INTEGER PRIMARY KEY AUTOINCREMENT,"
       "profile_id INTEGER NOT NULL UNIQUE,"
       "mapping_data TEXT,"
       "created_at INTEGER NOT NULL);");

  exec("CREATE TABLE IF NOT EXISTS devices_v3("
       "id INTEGER PRIMARY KEY AUTOINCREMENT,"
       "device_name TEXT,"
  ...
  exec("CREATE TABLE IF NOT EXISTS game_overrides_v3("
       "id INTEGER PRIMARY KEY AUTOINCREMENT,"
...
SqliteControllerRepository::getDeviceInfo(DeviceIdentifier identifier) const {
  QSqlQuery query(getDatabase());
  query.prepare("SELECT * FROM devices_v3 WHERE vendor_id = :vendorId AND "
...
void SqliteControllerRepository::updateDeviceInfo(DeviceIdentifier identifier,
  ...
  QSqlQuery query(getDatabase());
  query.prepare(
      "INSERT OR REPLACE INTO devices_v3 (device_name, device_type, vendor_id, "
...
void SqliteControllerRepository::loadProfileContents(
  ...
  const auto syncShortcuts = [id = profile->getId(),
    ...
    updateQuery.prepare("UPDATE shortcuts_v3 SET mapping_data = :mappingData "
  ...
  shortcutQuery.prepare(
      "SELECT * FROM shortcuts_v3 WHERE profile_id = :profileId");
  ...
  if (shortcutQuery.exec() && shortcutQuery.next()) {
    ...
    createShortcutsQuery.prepare(
        "INSERT INTO shortcuts_v3 (profile_id, mapping_data, created_at) "
...
SqliteControllerRepository::createProfile(const std::string name) {
  ...
  QSqlQuery query(getDatabase());
  query.prepare("INSERT INTO profiles_v3 (name, kind, builtin, based_on_type, "
...
SqliteControllerRepository::getProfile(const int id) {
  ...
  QSqlQuery query(getDatabase());
  query.prepare("SELECT * FROM profiles_v3 WHERE id = :id");
...
SqliteControllerRepository::listProfiles() {
  ...
  QSqlQuery query(getDatabase());
  query.prepare("SELECT id FROM profiles_v3 ORDER BY id");
  if (!query.exec()) {
...
bool SqliteControllerRepository::deleteProfile(const int id) {
  ...
  QSqlQuery query(getDatabase());
  for (const char *sql :
       {"DELETE FROM profiles_v3 WHERE id = :id",
        "DELETE FROM mappings_v3 WHERE profile_id = :id",
        "DELETE FROM shortcuts_v3 WHERE profile_id = :id"}) {
    query.prepare(sql);
    query.bindValue(":id", id);
    if (!query.exec()) {
...
bool SqliteControllerRepository::renameProfile(const int id,
                                               std::string newName) {
  QSqlQuery query(getDatabase());
  query.prepare("UPDATE profiles_v3 SET name = :name, updated_at = :updatedAt "
                "WHERE id = :id");
...
void SqliteControllerRepository::setProfileAnalogSettings(
  ...
  QSqlQuery query(getDatabase());
  query.prepare("UPDATE profiles_v3 SET analog_json = :analog, updated_at = "
...
std::optional<int> SqliteControllerRepository::getGameProfileOverride(
  ...
  QSqlQuery query(getDatabase());
  query.prepare(
      "SELECT profile_id FROM game_overrides_v3 WHERE content_hash = :hash");
...
void SqliteControllerRepository::setGameProfileOverride(
  ...
  QSqlQuery query(getDatabase());
  query.prepare("INSERT OR REPLACE INTO game_overrides_v3 (content_hash, "
...
void SqliteControllerRepository::clearGameProfileOverride(
  ...
  QSqlQuery query(getDatabase());
  query.prepare("DELETE FROM game_overrides_v3 WHERE content_hash = :hash");
  query.bindValue(":hash", QString::fromStdString(contentHash));
  if (!query.exec()) {
...
std::shared_ptr<InputMapping> SqliteControllerRepository::getOrCreateMapping(
  ...
  const auto syncMapping = [this](InputMapping &m) {
    QSqlQuery updateQuery(getDatabase());
    updateQuery.prepare(
        "UPDATE mappings_v3 SET mapping_data = :mappingData WHERE id = :id");
    updateQuery.bindValue(":mappingData",
  ...
  mappingsQuery.prepare(
      "SELECT * FROM mappings_v3 WHERE profile_id = :profileId AND "
  ...
  insertQuery.prepare(
      "INSERT INTO mappings_v3 (profile_id, platform_id, controller_type, "
