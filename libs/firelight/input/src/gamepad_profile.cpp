...
void GamepadProfile::setIsKeyboardProfile(const bool isKeyboardProfile) { m_isKeyboardProfile = isKeyboardProfile; }
...
void GamepadProfile::setPresetId(std::string presetId) { m_presetId = std::move(presetId); }
...
std::shared_ptr<InputMapping> GamepadProfile::getMappingForPlatformAndController(const int platformId,
                                                                                 const int controllerType) const {

  for (const auto &mapping : m_mappings) {
    if (mapping->getPlatformId() == platformId && mapping->getControllerType() == controllerType) {
      return mapping;
    }
  }
  return nullptr;
...
void GamepadProfile::setShortcutMapping(std::shared_ptr<ShortcutMapping> mapping) {
  m_shortcutMapping = std::move(mapping);
}

std::shared_ptr<ShortcutMapping> GamepadProfile::getShortcutMapping() const { return m_shortcutMapping; }

const AnalogSettings &GamepadProfile::getDefaultAnalogSettings() const { return m_defaultAnalogSettings; }

void GamepadProfile::setDefaultAnalogSettings(const AnalogSettings &settings) { m_defaultAnalogSettings = settings; }

AnalogSettings GamepadProfile::getAnalogSettings(const int platformId, const int controllerType) const {
  const auto mapping = getMappingForPlatformAndController(platformId, controllerType);
