...
DiscordManager::DiscordManager(platforms::IPlatformService &platformService) : m_platformService(platformService) {}
...
void DiscordManager::initialize() {
  ...
  m_client.UpdateRichPresence(m_defaultActivity, [](const discordpp::ClientResult &result) {
...
void DiscordManager::startGameActivity(const std::string &contentHash, const std::string &displayName, int platformId,
  ...
  if (!iconUrl.empty() && iconUrl.starts_with("http://") || iconUrl.starts_with("https://")) {
    ...
    const std::string image =
        platform && !platform->discordImageOrSlug().empty() ? platform->discordImageOrSlug() : "firelight-logo-white";
  ...
  m_client.UpdateRichPresence(m_currentActivity, [](const discordpp::ClientResult &result) {
    if (result.Successful()) {
      spdlog::info("[Discord] Rich presence updated");
    } else {
