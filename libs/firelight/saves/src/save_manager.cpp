...
// Atomically data to path via a temp file + rename, so a failed or partial write can never corrupt a
// prior good file. Returns false on error
...
bool SaveManager::transferSaves(const std::string &fromContentHash, const std::string &toContentHash) {
  ...
  // Only move files that don't already exist at the destination, to avoid overwriting saves
