...
/**
 * TODO: Probably should be moved to util, but for now it's only used here
 */
class Md5 {
public:
  Md5() { reset(); }
  ...
  std::string hexDigest() {
    ...
    const auto index = m_count >> 3 & 0x3f;
  ...
  static std::string hash(const void *data, const size_t len) {
    Md5 md5;
    md5.update(data, len);
