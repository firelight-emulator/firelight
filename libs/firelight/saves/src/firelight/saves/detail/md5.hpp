...
private:
  ...
  void transform(const uint8_t block[64]) {
    ...
    auto ff = [](uint32_t &a, uint32_t b, uint32_t c, uint32_t d, uint32_t x,
                 int s, uint32_t t) {
      a = b + rotl(a + ((b & c) | (~b & d)) + x + t, s);
    };
    auto gg = [](uint32_t &a, uint32_t b, uint32_t c, uint32_t d, uint32_t x,
                 int s, uint32_t t) {
      a = b + rotl(a + ((b & d) | (c & ~d)) + x + t, s);
    };
    auto hh = [](uint32_t &a, uint32_t b, uint32_t c, uint32_t d, uint32_t x,
                 int s, uint32_t t) {
      a = b + rotl(a + (b ^ c ^ d) + x + t, s);
    };
    auto ii = [](uint32_t &a, uint32_t b, uint32_t c, uint32_t d, uint32_t x,
                 int s, uint32_t t) {
