...
namespace firelight::library {
  namespace {
    ...
    Entry deserializeEntry(const SQLite::Statement &query) {
      return Entry{
        ...
        query.getColumn("boxart_front_source_url").getString(),
        .boxartBackSourceUrl =
        query.getColumn("boxart_back_source_url").getString(),
        ...
        query.getColumn("retroachievements_set_id").getUInt(),
        .createdAt =
        static_cast<uint64_t>(query.getColumn("created_at").getInt64()),
      };
    }
    ...
    ContentFile deserializeContentFile(SQLite::Statement &query) {
      return ContentFile{
        ...
        .m_fileSizeBytes =
        static_cast<size_t>(query.getColumn("file_size").getInt64()),
  ...
  SqliteUserLibraryRepository::SqliteUserLibraryRepository(QString path)
    : m_databasePath(path.toStdString()) {
    m_db = std::make_unique<SQLite::Database>(
      m_databasePath, SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE);
  ...
  void SqliteUserLibraryRepository::ensureColumnExists(const std::string &table,
    ...
    try {
      ...
      m_db->exec("ALTER TABLE " + table + " ADD COLUMN " + column + " " +
                 definition + ";");
    } catch (const std::exception &e) {
  ...
  int SqliteUserLibraryRepository::resolveContentDirectoryId(
    ...
    for (const auto &dir: getContentDirectories()) {
      if (startsWith(onDiskPath, dir.path) &&
    ...
    for (const auto &cf: getContentFiles()) {
      ...
      try {
        SQLite::Statement upd(*m_db,
                              "UPDATE content_files SET content_directory_id = "
                              ":dirId WHERE id = :id;");
  ...
  bool SqliteUserLibraryRepository::create(FolderInfo &folder) {
    ...
    try {
      ...
      SQLite::Statement query(
        *m_db, "INSERT INTO folders(display_name, description, icon_source_url, "
  ...
  bool SqliteUserLibraryRepository::deleteFolder(int folderId) {
    ...
    try {
      ...
      SQLite::Statement deleteEntriesQuery(
        *m_db, "DELETE FROM folder_entries WHERE folder_id = :folderId;");
  ...
  bool SqliteUserLibraryRepository::update(FolderInfo &folder) {
    ...
    try {
      ...
      if (query.exec() == 0) {
        spdlog::error("Failed to update folder with ID {}: no rows affected",
                      folder.id);
        return false;
      }
      return true;
  ...
  bool SqliteUserLibraryRepository::reorderFolders(
    ...
    try {
      ...
      for (size_t i = 0; i < orderedFolderIds.size(); ++i) {
        SQLite::Statement query(
          *m_db, "UPDATE folders SET position = :position "
  ...
  bool SqliteUserLibraryRepository::setFolderParent(const int folderId,
    ...
    try {
      ...
      query.bind(":folderId", folderId);
      if (query.exec() == 0) {
        spdlog::error("Failed to set parent of folder {} to {}: no rows affected",
  ...
  bool SqliteUserLibraryRepository::deleteFolderEntry(FolderEntryInfo &info) {
    ...
    try {
      SQLite::Statement query(*m_db,
                              "DELETE FROM folder_entries WHERE folder_id = "
  ...
  bool SqliteUserLibraryRepository::update(Entry &entry) {
    ...
    try {
      SQLite::Statement query(
        *m_db, "UPDATE entriesv1 SET display_name = :displayName, "
      ...
      if (query.exec() == 0) {
        spdlog::error("Failed to update entry with ID {}: no rows affected",
                      entry.id);
        return false;
      }
    } catch (const std::exception &e) {
  ...
  bool SqliteUserLibraryRepository::updateEntryMetadata(const Entry &entry) {
    ...
    try {
      ...
      spdlog::error("Failed to update entry metadata for ID {}: {}", entry.id,
                    e.what());
      return false;
    }
  ...
  bool SqliteUserLibraryRepository::deleteContentDirectory(int id) {
    ...
    try {
      SQLite::Statement selectQuery(
        *m_db, "SELECT path FROM content_directoriesv1 WHERE id = :id;");
    ...
    try {
      SQLite::Statement query(*m_db,
                              "DELETE FROM content_directoriesv1 WHERE id = :id;");
      query.bind(":id", id);
      query.exec();
    } catch (const std::exception &e) {
    ...
    EventDispatcher::instance().publish(
      ContentDirectoryRemovedEvent{.id = id, .path = path});
    return true;
  }
  ...
  bool SqliteUserLibraryRepository::create(ContentFile &romFile) {
    ...
    try {
      SQLite::Statement query(
        *m_db, "INSERT INTO content_files (file_path, file_size, file_md5, "
  ...
  SqliteUserLibraryRepository::getContentFileWithPathAndSize(
    const std::string &filePath, const size_t fileSizeBytes,
    const bool inArchive) {
    std::lock_guard lock(m_mutex);

    try {
      SQLite::Statement query(
        *m_db, "SELECT * FROM content_files WHERE file_path = :filePath AND "
      ...
      return ContentFile{
        .m_id = query.getColumn("id").getInt(),
        .m_type =
        static_cast<ContentType>(query.getColumn("content_type").getInt()),
        .m_fileSizeBytes =
        static_cast<size_t>(query.getColumn("file_size").getInt64()),
        .m_filePath = query.getColumn("file_path").getString(),
        .m_fileMd5 = query.getColumn("file_md5").getString(),
        ...
        query.getColumn("content_directory_id").getInt(),
      };
    } catch (const std::exception &e) {
  ...
  bool SqliteUserLibraryRepository::deleteContentFile(int id) {
    ...
    try {
      SQLite::Statement query(
        *m_db, "SELECT content_hash FROM content_files WHERE id = :id;");
      query.bind(":id", id);
      if (!query.executeStep()) {
        return true; // Nothing to delete
      }
      ...
      SQLite::Statement deleteQuery(*m_db,
                                    "DELETE FROM content_files WHERE id = :id;");
      deleteQuery.bind(":id", id);
      deleteQuery.exec();

      SQLite::Statement deleteRunConfigsQuery(
        *m_db, "DELETE FROM run_configurations WHERE type = 'rom' AND "
        "content_file_id = :contentFileId;");
      deleteRunConfigsQuery.bind(":contentFileId", id);
      deleteRunConfigsQuery.exec();

      SQLite::Statement deleteDiscMembersQuery(
        *m_db, "DELETE FROM disc_members WHERE content_file_id = :id;");
    ...
    EventDispatcher::instance().publish(
      RunConfigurationDeletedEvent{.contentHash = contentHash});
    return true;
  }

  std::vector<Entry> SqliteUserLibraryRepository::getEntries(int offset,
                                                             int limit) {
    std::lock_guard lock(m_mutex);
  ...
  std::optional<Entry> SqliteUserLibraryRepository::getEntry(const int entryId) {
    ...
    try {
      SQLite::Statement query(
        *m_db, "SELECT * FROM entriesv1 WHERE id = :entryId LIMIT 1;");
  ...
  std::optional<Entry> SqliteUserLibraryRepository::getEntryWithContentHash(
  ...
  void SqliteUserLibraryRepository::populateEntrySource(Entry &entry) {
    try {
      SQLite::Statement query(
        *m_db, "SELECT content_directory_id, file_path, in_archive, "
      ...
      while (query.executeStep()) {
        ...
        if (dirId >= 0 && std::ranges::find(entry.contentDirectoryIds,
                                            dirId) == entry.contentDirectoryIds.end()) {
          entry.contentDirectoryIds.push_back(dirId);
        }

        const auto path = query.getColumn("in_archive").getInt() != 0
                            ? query.getColumn("archive_file_path").getString()
  ...
  SqliteUserLibraryRepository::getRunConfigurations(
    const std::string &contentHash) {
    std::lock_guard lock(m_mutex);
    ...
    try {
      ...
      while (query.executeStep()) {
        runConfigurations.push_back(RunConfiguration{
          ...
          .createdAt =
          static_cast<uint32_t>(query.getColumn("created_at").getInt64()),
        });
      }
    } catch (const std::exception &e) {
  ...
  std::optional<ContentFile> SqliteUserLibraryRepository::getContentFile(int id) {
    ...
    try {
      SQLite::Statement query(
        *m_db, "SELECT * FROM content_files WHERE id = :id LIMIT 1;");
      query.bind(":id", id);
  ...
  bool SqliteUserLibraryRepository::create(DiscMember &member) {
    ...
    try {
      SQLite::Statement query(
        *m_db, "INSERT OR IGNORE INTO disc_members (content_file_id, path, "
  ...
  std::optional<PatchFile> SqliteUserLibraryRepository::getPatchFile(int id) {
    ...
    try {
      SQLite::Statement query(*m_db,
                              "SELECT * FROM patch_files WHERE id = :id LIMIT 1;");
      query.bind(":id", id);
  ...
  void SqliteUserLibraryRepository::create(PatchFile &file) {
    ...
    try {
      SQLite::Statement query(
        *m_db, "INSERT OR IGNORE INTO patch_files (file_path, file_size, "
  ...
  std::vector<ContentDirectory>
  SqliteUserLibraryRepository::getContentDirectories() {
    std::lock_guard lock(m_mutex);
    ...
    try {
      ...
      while (query.executeStep()) {
        directories.emplace_back(ContentDirectory{
          ...
          .lastModifiedEpochMs =
          static_cast<uint64_t>(query.getColumn("last_modified").getInt64()),
  ...
  bool SqliteUserLibraryRepository::create(ContentDirectory &directory) {
    ...
    try {
      SQLite::Statement query(
        *m_db, "INSERT OR IGNORE INTO content_directoriesv1 (path, created_at) "
  ...
  bool SqliteUserLibraryRepository::update(const ContentDirectory &directory) {
    ...
    try {
      ...
      SQLite::Statement query(
        *m_db,
        "UPDATE content_directoriesv1 SET path = :path WHERE id = :id;");
      query.bind(":path", directory.path);
      query.bind(":id", directory.id);
      query.exec();
    } catch (const std::exception &e) {
      spdlog::error("Failed to update content directory {}: {}", directory.id,
                    e.what());
      return false;
    }

    EventDispatcher::instance().publish(
      ContentDirectoryUpdatedEvent{
        .id = directory.id,
        .oldPath = oldPath,
  ...
  void SqliteUserLibraryRepository::createRunConfiguration(
    const int contentFileId, const std::string &path, const int platformId,
    const std::string &contentHash) {
    std::lock_guard lock(m_mutex);
    int64_t rowId = 0;
    ...
    try {
      SQLite::Statement query(
        *m_db, "INSERT OR IGNORE INTO run_configurations "
    ...
    EventDispatcher::instance().publish(
      RunConfigurationCreatedEvent{
        .id = static_cast<int>(rowId),
        .filePath = path,
  ...
  bool SqliteUserLibraryRepository::createEntry(Entry &entry) {
    ...
    try {
      SQLite::Statement selectQuery(
        *m_db, "SELECT id FROM entriesv1 WHERE content_hash = :contentHash;");
      ...
      SQLite::Statement entryQuery(
        *m_db, "INSERT INTO entriesv1 (display_name, content_hash, platform_id, "
