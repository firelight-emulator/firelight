...
Entry deserializeEntry(SQLite::Statement &query) {
  return Entry{
...
void SqliteUserLibraryRepository::ensureColumn(const std::string &table,
...
SqliteUserLibraryRepository::getContentFileWithPathAndSize(
  ...
  try {
    ...
    // Preserves the original field set (crc32 intentionally left unset here).
    return ContentFile{
        .m_id = query.getColumn("id").getInt(),
        .m_type =
...
void SqliteUserLibraryRepository::createRunConfiguration(
  ...
  EventDispatcher::instance().publish(
      RunConfigurationCreatedEvent{.id = static_cast<int>(rowId),
                                   .filePath = path,
