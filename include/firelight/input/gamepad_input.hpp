...
constexpr unsigned retroDeviceId(const GamepadInput input) { return static_cast<unsigned>(input) & 0xFFu; }
...
inline const std::array<GamepadInput, 16> &modifierCandidates() {
  static constexpr std::array<GamepadInput, 16> CANDIDATES = {
      SouthFace, EastFace, WestFace, NorthFace, LeftBumper, RightBumper, LeftTrigger, RightTrigger,
      L3,        R3,       Select,   Start,     DpadUp,     DpadDown,    DpadLeft,    DpadRight};
  return CANDIDATES;
}
