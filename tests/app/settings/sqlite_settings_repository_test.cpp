...
protected:
  ...
  void SetUp() override { repository = std::make_unique<SqliteSettingsRepository>(":memory:"); }
...
TEST_F(SqliteSettingsRepositoryTest, GetEffectiveValue_ResolutionOrder) {
  ...
  repository->setGameValue(contentHash, key, "game");
  EXPECT_EQ(repository->getEffectiveValue(contentHash, platformId, key, def), "game");
