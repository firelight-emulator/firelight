...
public:
  ...
  void setConfigProvider(std::shared_ptr<firelight::libretro::IConfigurationProvider> provider) {
  ...
  void init() override {
    ...
    if (m_configProvider) {
      ...
      a.possibleValues = {{.key = "on", .label = "On"}, {.key = "off", .label = "Off"}};
  ...
  // --- save data / state ---
  void writeMemoryData(::libretro::MemoryType type, const std::vector<char> &data) override {
    if (type == ::libretro::SAVE_RAM) {
      m_sram = data;
    }
  }

  [[nodiscard]] std::vector<char> getMemoryData(::libretro::MemoryType type) const override {
  ...
  [[nodiscard]] std::size_t getSerializeSize() const override { return sizeof(int) + m_sram.size(); }
  ...
  [[nodiscard]] const std::vector<uint8_t> &systemRam() const { return m_systemRam; }
  ...
  [[nodiscard]] unsigned getCurrentDiskIndex() const override { return m_discIndex; }
  ...
  void setControllerDevices(std::vector<std::vector<ControllerDeviceOption>> devices) {
    m_controllerDevices = std::move(devices);
  }

  [[nodiscard]] std::vector<std::vector<ControllerDeviceOption>> getControllerDevices() const override {
  ...
  void sendKeyboardEvent(const bool down, const unsigned key, uint32_t, uint16_t) override {
  ...
  [[nodiscard]] const std::vector<CheatCall> &cheatCalls() const { return m_cheatCalls; }
  ...
  [[nodiscard]] const std::string &savedSaveDirectory() const { return m_saveDir; }

  [[nodiscard]] const std::vector<std::pair<unsigned, unsigned>> &portDeviceCalls() const { return m_portDeviceCalls; }
