...
public:
  ...
  void setHotkeysEnabled(const bool enabled,
                         const std::optional<DeviceType> only = {}) override {
  ...
  std::vector<std::shared_ptr<input::IGamepad>> listGamepads() override {
    return {};
  }
  std::shared_ptr<input::IGamepad> getPlayerGamepad(int) override {
    return nullptr;
  }
  ...
  std::shared_ptr<libretro::IRetroPad> getRetropadForPlayerIndex(int) override {
    return nullptr;
  }
  std::pair<int16_t, int16_t> getPointerPosition() const override {
    return {0, 0};
  }
